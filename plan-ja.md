...
1. **Buchholz の場合 4 の塔不変量。** `TowerOT` で、ライブラリが仮定している
   唯一のものである。場合 4 の配置にある `ψ_A(B)` について、塔の各段 `W_i` が
   標準形であり、`G_A` がそこに見るものが `B[W_i]` より下にある、という主張で
   ある。`exb.WF` への道筋の他のものは全部証明済みである。3.2(b)、3.4、3.5、
   `SubBound`、3.6、そして 3.3 自身も。`test/ExBuchholzCheck.lean` で、サイズ 8
   以下の標準形で場合 4 の形 651 個について 5 段まで計算により確認してある。
   その先も確認済みで、サイズ 8 以下の可算標準形 3835 個を `0`〜`4` のどれで展開
   しても標準形・可算・狭義減少が保たれる。サイズ 9（15890 個）でも同じ。サイズ
   6 以下の標準形は、定義域より下のサイズ 3 以下の標準形で展開しても標準形の
   ままである。ε₀、ψ_0(Ω+Ω)、ψ_0(ψ_1(1))、ψ_0(Ω_2)、ψ_0(ψ_Ω(0)) は途中の項が
   全部標準形のまま `0` に到達する
