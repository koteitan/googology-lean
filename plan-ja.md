...
`Arg.lean` と `EpsBig.lean` が `e_{e0}` まで運ぶ。
...
* **`e_{e0}` 以上での `val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への
  ...
  `e_{e0}` と `z0` の間に残るのは、`e0` 以上での引数の項である。`exists_argTerm` は
  `mu` の指数を Cantor 標準形から読み、その指数を名指す all-nil の項を要求する。それを
  与えるのが `exists_desc_of_lt_eps0` である。`e0` より上では指数自身が e 数になりうる。
  それを名指す項は自分の添字より上の段でしか作れないので、引数の項と値の項を別々に
  作るのではなく一本の帰納法で作る必要がある。e の逆関数が要るように見える箇所は要らない。
  `Ord.exists_eps_index` が `Ordinal.le_iff_deriv` から添字を出し、`Ord.eps_index_lt` が
  それが小さいと言う。そこまで組んで届く先が `z0` である。`z0` は `p0(W·z0)` であり、
  `p0` と `p1` が二つ揃って名指せない最初の順序数、`e0` の二段版である。
