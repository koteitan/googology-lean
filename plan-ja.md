...
2. **1 行については完了: BMS がどの順序数を名指すか。** `Trans/BMS/` に一本の
   ...
   仮定は両方 `rfl` である。これらが届くのは `1` と `e0` から `+` と `·w` で作られる
   順序数である。届かないのは `(0,0)(1,1)(2,2)` 以降の生成元で、そちらの展開は 0 の
   行の行列でもブロックの繰り返しでもない。
...
* **`e0` 以上での `val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への
  順序**同型**だと述べている。単調で単射という半分はここにある（`val_lt_val`、
  `val_inj_of_OT`）。全射性を証明したのは `e0` 未満だけである。その上の算術には
  着手した。`psi_Omega_add_eq` が `e1` まで `p0(W + a) = e0·w^a` だと言い、
  `psi_Omega_two` が `p0(W·2)` は `e1` **そのもの**だと言う。`e1` 未満の全射性を
  証明するなら項はこれで組む。引数に `W` が入る項の標準形条件は
  `OT_psi_Omega_add` である。要求は、引数の中に `G_0` が見るもので `W + B` に届く
  ものが無いこと。`p0(W+1)` と `p0(W+W)` はこれで標準形になり、`e0·w` と `e1` を
  名指す。残るのは帰納法そのもので、その `G_0` の条件を値と一緒に運ぶ必要がある。
  部品は二つ入った。`OT_cons_Omega` が和を作り、`Ord.log_lt_self_of_lt_eps1` が
  再帰に要る減少を与える。`e0` より上で `w ^ ·` の不動点は `e1` 自身しかなく、
  それは `Ord.eps1_le_of_opow_fp` が排除する。さらに上では標準形が Cantor のものではなく、
