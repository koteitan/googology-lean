...
* **`val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への順序**同型**
  ...
  なら Cantor 標準形である。`α < ε₀` はどれも `ω^a₁ + ⋯ + ω^aₙ` で各 `aᵢ < α` だから、
  `α` についての帰納法で項が作れる。ただし `α < Ω` での `ψ_0(α) = ω^α` が要り、
  それは `Ord.lean` に無い。そこまで行けば、1 行は `ε₀` 未満の順序数を項としてだけ
  でなく順序数として**ちょうど**名指すことになる。逆向きは `lt_e0_iff_allNil` と
  `exists_read` で既にある。
