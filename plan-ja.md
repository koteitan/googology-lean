...
1. **Bachmann 性。** `Bachmann` で、ライブラリが仮定している唯一のものである。
   Buchholz の場合 4 の配置にある `ψ_A(B)` について、`G_A` が `B` に見るものが
   全部、塔の最初の値 `B[ψ_{Z[0]}(0)]` より下にある、という主張である。
   `exb.WF` への道筋の他のものは全部証明済みである。3.2(b)、3.4、3.5、
   `SubBound`、3.6、場合 4 の塔不変量、そして 3.3 自身も。
   `test/ExBuchholzCheck.lean` で、サイズ 8 以下の標準形で場合 4 の形 651 個に
   ついて計算により確認してある。その先も確認済みで、サイズ 8 以下の可算標準形
   3835 個を `0`〜`4` のどれで展開しても標準形・可算・狭義減少が保たれる。
   サイズ 9（15890 個）でも同じ。サイズ 6 以下の標準形は、定義域より下のサイズ
   3 以下の標準形で展開しても標準形のままである。ε₀、ψ_0(Ω+Ω)、ψ_0(ψ_1(1))、
