...
1. **`OT` と `· < Ω` が一歩の展開で保たれること。** Buchholz 補題 3.3 の拡張版で、
   ...
   どちらも `Closure.lean` にある。3.6 は `Trian_fs` で、残るのは `TowerBound`
   という 1 つの主張だけである。これは、Buchholz の場合 4 の塔の各段に `G` が
   見るものが、その段の生む値と `X₂` の間にある項で抑えられる、という主張で
   ある。3.6 が 3.3 を呼ぶのはここだけで、そのため 2 つを 1 つの同時帰納法で回す
   必要がある。評価は `c` に相対でなければならない。Buchholz の絶対形は、添字が
   項になると偽になる。それを壊す項と `TowerBound` 自体の確認は
   `test/ExBuchholzCheck.lean` にある。3.3 も同じファイルで計算により確認済み。
