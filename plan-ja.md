...
* **`e_w` 以上での `val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への
  ...
  いまは順序数の側が項の側より先に行っている。`val` の全射性は `e_w` 未満まで、
  算術は `z0` 未満まで決まっている。項の側が追いつくのに要るのは **`W·(1+g)` を
  名指す項**である。`W·w^e` は `p1(e)` である（`Ord.psi_one_eq`）。だから Cantor
  標準形が `W·mu` を `p1` の項の和に直す。その引数は指数を名指す項で、指数はより
  小さい順序数だから、この構成は値の側の構成の後ではなく、並行して回る。引数の項が
  手に入れば、残りは `EpsN.lean` の構成を一段上げたものである。先頭項
  `p0(W·(1+g) + B)` を標準形の言う回数だけ繰り返す。運ぶ不変量は、引数の項の `G_0`
  がその項より下に留まることである。

  その帰納法は値ではなく `g` について回り、引数の項を一緒に運ぶ。`g = 0` では引数が
  `W` で、`e0` 未満の値は Cantor 標準形である。`g + 1` では引数に `W` が一つ増える。
  極限の `g` では `g` の Cantor 標準形から組み直し、その指数はより低い段で名指されて
  いる。e の逆関数が要るように見える一箇所 — 指数自身が e 数のとき、それは自分の
  添字より上の段でしか名指されない — は要らない。`Ord.exists_eps_index` が
  `Ordinal.le_iff_deriv` から添字を出し、`Ord.eps_index_lt` がそれが小さいと言う。
  全部を組んで届く先が `z0` である。`z0` は `p0(W·z0)` であり、`p0` と `p1` が二つ
  揃って名指せない最初の順序数、`e0` の二段版である。
