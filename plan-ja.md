...
2. **1 行については完了: BMS がどの順序数を名指すか。** `Trans/BMS/` に一本の
   ...
   照合はしたが証明していないこと: `prim` の状態、つまり項が標準形である行列が、
   参照実装の 1 行標準形とちょうど一致すること。長さ 5、成分 4 未満の 1024 通り
   すべてで両者は一致する（`test/TransCheck.lean`）。証明するには、そのような
   行列がすべて生成元から到達可能であることを示す必要がある。

   2 行は Bachmann–Howard 順序数未満、3 行以上は未解決なので、次の目標は `r = 2`
   である。具体的に要るもの: `BMS/OneRow.lean` の 2 行版。`m₀` は 0 か 1 で、
   `m₀ = 1` のとき行 0 に加算が入る。`p0` だけでなく `p1` も使う読み取り。そして
   可換性。その `dom` の三分法には 1 行では決して現れない第 4 の場合、タワーが
   出てくる
