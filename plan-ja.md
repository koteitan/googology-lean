...
3b は 4 を要ります。`G a (cons c d r)` は `a ≤ c` で場合分けし、もう一方の枝では
構文的に `c < a` が成り立っていて、証明には `val c < val a` が要ります。それが 4 で
あります。逆に 4 も 3b を要ります。2 つの主項が添字を共有する場合です。

だからこれは 1 つの同時帰納になります。測度は `size x + size y` で、どの呼び出しも
狭義に下がります。
...
* `#guard` の行は小さい場合の計算であって定理ではありません。見た目で分かるように分けて
  ある
