...
1. **`OT` と `· < Ω` が一歩の展開で保たれること。** Buchholz 補題 3.3 の拡張版で、
   ライブラリと `exb.WF` の間に残る唯一のものである。この道筋の他のものは
   全部証明済みである。3.2(b)、3.4、3.5、そして 3.6 自身も、`Closure.lean` が
   3.3 だけから出している。`test/ExBuchholzCheck.lean` で計算により確認済み。
   サイズ 8 以下の可算標準形 3835 個を `0`〜`4` のどれで展開しても標準形・可算・
   狭義減少が保たれる。サイズ 9（15890 個）でも同じ。サイズ 6 以下の標準形は、
   定義域より下のサイズ 3 以下の標準形で展開しても標準形のままである。ε₀、
