...
* **`e1` 以上での `val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への
  ...
  さらに上へ行くには `e1` 以上の算術が要り、そこは形が違う。`p0(W·2 + a)` は別の
  塔を始め、`p0(W^2)` は `z0` であり、いずれ引数に `p1` が入る。そうなると帰納法は
  `p1` がどの引数に届くかを知る必要があり、それは `C_1` そのものである。一段ずつ
  上げても終わらない。終わらせるのは正規形定理で、その第一歩は入った。
  `Ord.principal_mem_CSet` が、`C_v(a)` の加法的主要な元は `W_v` 未満か、閉包の中の
  `u`・`e` による collapse `p_u(e)` かのどちらかだと言う。残るのは、任意の元を主要な
  ものへ分解すること（Cantor 標準形と `mem_CSet_of_le`）と、各 `p_u(e)` を項に直す
  再帰である。後者が難所で、`e` は名指す順序数より大きいことがある。さらに上では標準形が Cantor のものではなく、
