...
| `Eps.lean` | 済。有限の全段での `p0(W·(n+1)) = e_n`。帰納法一本 |
...
* **`e1` 以上での `val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への
  ...
  そこが届かないのが `W·w` 以上で、理由は最後の条項である。`p1(1) = W·w` だから、
  `W·w` では添字 `1` で引数が `0` でない collapse を認めねばならず、それは `C_0` では
  なく `C_1` である。その先は `p0(W^2)` が `z0` であり、引数に `p1` が入る。そうなると
  帰納法は `p1` がどの引数に届くかを知る必要がある。全部を終わらせるのは正規形定理で、
  その第一歩は入った。
