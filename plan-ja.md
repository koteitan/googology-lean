...
2. **1 行については完了: BMS がどの順序数を名指すか。** `Trans/BMS/` に一本の
   ...
   ブロックについて加法的である（`rank_appendState`）。そこから 2 行の値が三つ出る。
   `(0,0)(1,1)` の `n` 個並びが `e0·n`、`(0,0)(1,1)(1,0)` が `e0·w` である。後者は
   ちょうどその並びへ展開する。まだ届かないのは `(0,0)(1,1)(2,2)` 以降の生成元で、
   そちらの展開は 0 の行の行列でもブロックの繰り返しでもない。
...
* **`e0` 以上での `val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への
  ...
  `val_inj_of_OT`）。全射性を証明したのは `e0` 未満だけである。その上では標準形が
  Cantor のものではなく、どの添字の `p` も要る。だから項を作る帰納法は、各 `p_v` が
  どの引数に届くかを知っていなければならず、それは `C_v` そのものである。
...
* **2 行以上の読み取り。** 行列から順序数への写像の定義文が要る。調べた資料は値を
  ...
  繰り返しである。`(0,0)(1,1)(2,1)` のような行列はそのどれでもない。
