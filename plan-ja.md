...
1. **`OT` と `· < Ω` が一歩の展開で保たれること。** Buchholz 補題 3.3 の拡張版で、
   ...
   見るものが、その段の生む値より下にある、という主張である。3.6 が 3.3 を呼ぶ
   のはここだけで、そのため 2 つを 1 つの同時帰納法で回す必要がある。鍵になる
   具体的な事実は `G_0(X₂) < X₂` で、標準形一般には成り立たないが場合 4 の配置
   では成り立つ。どちらも `test/ExBuchholzCheck.lean`
