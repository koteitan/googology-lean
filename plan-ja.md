...
* **`e_w` 以上での `val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への
  ...
  `W·w` そのものには届く。別の論法による。`Ord.psi_Omega_omega` が
  `p0(W·w) = e_w` と言う。こちらは分解を全く使わない。`W·w` 未満の引数はどれかの
  `W·(n+1)` 未満だから、その `p0` は単調性だけで `e_n` 以下になる。その上界を項の値
  として名指すのが `Ord.psi_one_one` である。`p1(1) = W·w` である。`C_1(1)` には `0`
  以外に collapse する引数が無く、`W_2` 未満ではその閉包は `W` の有限倍に可算なものを
  足したものだけだからである。

  止まるのは `W·w + 1` である。そこでは `p1(1) = W·w` 自身が `C_0(W·w + 1)` の中の
  正当な collapse になる。添字が `1`、引数が `1`、どちらも閉包の中にある。だから
  その閉包の分解は `p1` がどの順序数に届くかを言う必要があり、それは `C_0` ではなく
  `C_1` である。その先は `p0(W^2)` が `z0` で、引数にも `p1` が入る。全部を終わらせる
  のは正規形定理で、その第一歩は入った。
