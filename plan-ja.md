...
1. **`OT` と `· < Ω` が一歩の展開で保たれること。** Buchholz 補題 3.3 の拡張版で、
   ...
   どちらも `Closure.lean` にある。3.6 は `Trian_fs` で、`fs` の枝 1 つ
   （Buchholz の場合 4、`Case4` として仮定に置いたもの）を除いて証明済みである。
   その枝には、項だけでなく指標についての帰納法が要る。Buchholz では 3.3 と 3.6
   を同時に回すことでそれが出る。`test/ExBuchholzCheck.lean`
