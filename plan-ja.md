...
* **`val` の全射性。** 出典は `OT` に制限した `val` が `C_0(Λ)` への順序**同型**
  だと述べている。単調で単射という半分はここにある（`val_lt_val`、
  `val_inj_of_OT`）。全射性は証明していない。だから行列がどの順序数を名指すかに
  ついて、項の言葉では厳密に、順序数の言葉では片側だけしか言えていない。`ε₀` 未満
  なら道筋は Cantor 標準形である。`α < ε₀` はどれも `ω^b₁ + ⋯ + ω^bₙ` で
  `b₁ ≥ ⋯ ≥ bₙ`、しかも各 `bᵢ < α` になる。最小の ε 数が `ε₀` 自身だから、指数が
  自分の `α` に等しくなることはない。だから `α` についての帰納法で、添字が全部 `0`
  の標準形が作れる。降下条件は `Trans.BMS.OT_of_desc` である。`Ord.lean` から要るのは
  `ψ_0(β) = ω^β` だが、これは `β < Ω` 全部では成り立た**ない**。`ε₀ ∉ C_0(ε₀ + 1)`
  なので `ψ_0(ε₀ + 1) = ψ_0(ε₀) = ε₀` である一方 `ω^(ε₀+1) = ε₀·ω` である。成り立つのは
  `β ∈ C_0(β)` のときで、それは標準形の引数について `val_mem_CSet_arg` が言っている
  ことである。そこまで行けば、1 行は `ε₀` 未満の順序数を項としてだけでなく順序数
  として**ちょうど**名指すことになる。逆向きは `lt_e0_iff_allNil` と `exists_read`
  で既にある。
