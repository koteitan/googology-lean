...
2. `Trans/BMS/ExBuchholz` を足す。BMS の停止性にはもう要らないので、翻訳が買うのは
   **値**である。行列がどの順序数を名指すか。これは BM4 の順序数解析で、決着が
   ついているのは行数が少ないときだけである。1 行は原始数列系で ε₀ 未満、2 行は
   Bachmann–Howard 順序数未満、3 行以上は未解決である。だから届く目標はまず
   `r = 1` である。写像自体は見えている。数列を 0 で区切り、ブロック `0 s` を
   `ψ_0(⟨s を 1 つ下げたもの⟩)` に送ればよく、`OT` かつ `Ω` 未満に入ることは
   その形から出る。仕事は可換性の方である。`BM4.expand` は `Classical.choice`
   を通して述べられているので、`fs` と一歩ずつ突き合わせるところに手間がかかる
4. DBMS と Y 数列を足す
