...
一つは Lean の問題で、二つはそうではない。

* **階層の `r` から `r + 1` への一段。** `BMS/Embed.lean` が 1 行を 2 行の中に
  入れる。2 行の生成元 `(0,0)(1,1)` が 1 行の生成元に 0 の行が付いたものへ展開し、
  `BMS/Zero.lean` が規則はその余分な行に気づかないと言う。同じ議論はどの `r` でも
  走るはずである。`(0,…,0)(1,…,1)` は `N` で展開すると `stair r N` に 0 の行が
  付いたものになり、それは `test/TransCheck.lean` の 3 行の展開が示している。要るのは
  どの `r` でも成り立つ規則のレベルの主張で、それには `parent A k` と `anc A k` を
  二つの行数の間で突き合わせることになる。機械的で、まだ書いていない。
