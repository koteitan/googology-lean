...
- 整礎性：展開の関係に無限降下列が無い。標準形から始めたどの展開列も有限で止まる（停止性）ことと同値で、それも証明してある。
- 停止(非標準)：標準形でない配列から始めても止まる。
- 翻訳写像：各状態を順序数表記の項に写し、その値として状態が名指す順序数を与える写像がある。原始数列と DBMS 1 行では、値が展開の階数と一致し、`e0` 未満の順序数ちょうどを名指す。拡張ブーフホルツ ψ では項の値そのもので、標準形から `C_0(Λ)` への順序同型である（階数との一致は未証明）。
