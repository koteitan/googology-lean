...
|---|:-:|:-:|:-:|
| BMS、DBMS（任意の行数。原始数列、ペア数列、トリオ数列を含む） | ✅ | ✅ | ✅ |
| Y 数列（1-Y） | ✅ |  |  |
| 拡張ブーフホルツ ψ | ✅ | ✅ |  |
...
| DBMS（1 行） | ✅ |  | ✅ | ✅ | ✅ |  |
| 拡張ブーフホルツ ψ | ✅ | ✅ | ✅ | ✅ |  | ✅ |
| 2 行以上の BMS、DBMS（ペア数列、トリオ数列を含む）、Y 数列 |  |  |  |  |  |  |
...
|---|:-:|:-:|:-:|:-:|:-:|:-:|
| 原始数列 → 拡張ブーフホルツ ψ（`p0(W)` 未満） | ✅ | ✅ | ✅ | ✅ | ✅ |  |
| DBMS 1 行 → 原始数列 | ✅ | ✅ | ✅ |  |  |  |
| 原始数列 → ペア数列（下に 0 の行を足す） | ✅ | ✅ | ✅ |  |  |  |
| BMS `r` 行 → BMS `r+1` 行（下に 0 の行を足す） | ✅ | ✅ | ✅ |  |  | ✅ |
| DBMS `r` 行 → BMS `r` 行の全配列 | ✅ | ✅ |  |  |  |  |
| 拡張ブーフホルツ ψ の `p0(W_a)`（`a < e0`）→ トリオ数列 | ✅ |  |  |  |  |  |

- 定義：写像を Lean で定義してある。最後の行は [koteitan/trio](https://github.com/koteitan/trio) の写像の書き起こしで、定義したうえで対応表と照合しただけである。
