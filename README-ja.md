...
- (*2) ペア数列 → 拡張ブーフホルツ ψ の「展開を保つ」と「展開と可換」は偽である（反証済み）。生成元 `(0,0)(1,1)` を `[0]` で展開すると `(0,0)` になり、それぞれ `p0(W_1)` と `1` に写る。`p0(W_1)` の基本列の項は `1` にならない（`Trans/PSS/Expansion.lean`）。ただし 1 手は、拡張ブーフホルツ ψ の側の 1 手以上に写る。「何手かで届く」は写像で両方向に保たれる（`pairToExb_transGen_iff`、`Trans/PSS/Steps.lean`）。例：`(0,0)(1,1)[0]` は `p0(W_1) →[0] w →[1] 1` に写る。
