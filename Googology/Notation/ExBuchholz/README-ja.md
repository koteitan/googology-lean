...
| Veblen の記法で | `p0(W·(1+g)) = phi_1(g)`、`p0(W^2) = phi_2(0)` | `psi_Omega_mul_veblen`, `psi_Omega_sq_veblen` |
...
出典は、`o` の `OT` への制限が `C_0(Λ)` への順序**同型**だと述べている。これは
...
閉包に入るが、その標準形の引数は自分の閉包に入っていなければならず、その引数は
一般に `e` より大きい。`p0(e0) = p0(W)` で、標準形が使うのは `W` である。そこで
引数を
...
* `p_u(M(e)) = p_u(e)` であり、`M(e)` は自分の閉包に入る。`psi_M_eq` と
  `M_mem_self` である。どちらも短い。`C_u(e)` の元は `[e, M(e))` に無いからである。
* `M(e)` はまた標準形の値である。`M_mem_of_comp` が、ある順序数の `M` を Cantor
  標準形の各項の `M` に帰着させる。先頭項 `p` が閉包に入っていれば残り、
  `M(x) = p + M(残り)` となる。入っていなければ `M(x) = M(p)` となる。
  `M_psi_mem` が項 `p_t(h)` を扱う。その上の閉包の次の元は `p_t(M(h))`、
  `W_{t+1}`、`W_{M(t)}` のどれかである。ここで作る collapse の引数は `e` 未満
  なので、`e` についての帰納法がそれを与える。
* `M(e)` が自分の閉包に入ることから、構文的な条件 `G_u(M(e)) < M(e)` が出る。
  これが `G_lt_of_mem_CSet` で、`Mono.main` の閉包の半分の逆である。その collapse
  の場合が `arg_mem_of_psi_mem` である。`v <= w` で `p_w(d)` が `C_v(b)` に入り、
  `d` が自分の閉包に入るなら、`d` は `C_v(b)` に入り `d < b` である。使うのは
  同じ `M` で、値の集合の代わりに `S = C_v(b)` を置く。

`Trans/BMS/` の、`e1`、`e_w`、`e_{e0}`、`z0` へ登る構成が先にあり、いまも残して
...
| **`val` が `C_0(Λ)` の上へ全射であること** | **済**（`Vals_eq`、`valEquiv`） |
