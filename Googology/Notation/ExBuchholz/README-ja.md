...
主項の 3 枝のうち、`ψ_a(0)` で `dom a = 1` の枝が `bach_succ` で、証明済みで
ある。`x` が前置きの下か、等しいか、上かで場合を分け、3 番目が
`le_pred_of_lt` とサイズの議論で閉じる。

残る 2 枝は落ちるが、落ちる先が別の文脈での主張になる。`ψ_a(0)` で
`dom a ∉ {0,1}` の枝は、同じ前置きのもとでの `a` での `P` 形になる。
`G_u(ψ_a(0))` が `{0}` と `G_u(a)` で、結論が `ψ_{a[W]}(0)` についてだからで
ある。`ψ_a(b)` で `dom b < V` の枝は 3 つに分かれる。`G_u(a)` はサイズの議論、
`G_u(b)` は文脈が `p + ψ_a(−)` の形、引数 `b` そのものは **水準 `a` での** `b`
での素の形で、その仮定はちょうど `OT (ψ_a(b))` に対する `OT_G_lt` である。
最後のところがあるので、帰納法は水準を固定せず全称にしておく必要がある。

文脈は閉じない。`P` 形の和の枝は `ψ_{p + ψ_α(β) + (−)}(0)` を要求し、以下同様
になる。だから帰納法は 3 つの形ではなく一般の文脈を回したい。文脈にどういう
閉包条件を置けばよいかを見つけるのが、残っている仕事である。
