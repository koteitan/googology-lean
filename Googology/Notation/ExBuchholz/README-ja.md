...
この評価は `c` に相対でなければならない。Buchholz は不変量を絶対形
`G_u(W_i) < X₂[W_i]` で書いている。彼の体系では添字が数なので `G` が添字の中に
入らず、それで済む。拡張版では添字が項で `G` が中に入るため、絶対形は偽である。
`A = ψ_0(ψ_Ω(0))`、`X = ψ_Ω(ψ_{A+1}(0))` とすると、塔の第 0 段は `ψ_A(0)` で、
その段の生む値も `ψ_A(0)` だが、`G_0` はそこに `ψ_Ω(0)` を見る。`A` は可算なので
`ψ_Ω(0) > ψ_A(0)` である。

`test/ExBuchholzCheck.lean` にその項を置いてある。`TowerBound` 自体は、サイズ 7
以下の標準形で場合 4 の形 158 個（可算とは限らない）について、4 段、サイズ 2
以下の全レベル、サイズ 4 以下の全候補 `c` で確認してある。サイズ 8（651 個）でも
同じ結果である。
...
| `TowerBound` そのもの | **未証明**。場合 4 の形 158 個について計算で確認 |
