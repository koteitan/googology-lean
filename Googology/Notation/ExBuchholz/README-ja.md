...
主項の 3 枝はまだ書いていない。どれも `x` が前置き `p` の下か、等しいか、上か
で場合を分ける。仕事があるのは 3 番目だけで、そこでは `x = p + x'` となり、
`x'` はどの `G` の要素でもないので帰納法の仮定が届かない。`ψ_a(0)` で
`dom a = 1` の枝では `le_pred_of_lt` とサイズの議論で閉じる。これが
`bach_succ` で、証明済みである。`ψ_a(0)` で
`dom a ∉ {0,1}` の枝では、そもそもこの場合が起きないように見える。残るのは
`ψ_a(b)` で `dom b < V` の枝である。ここでは `x'` の頭の添字が `a` になることが
あり、そのときはその頭の引数が `b[W]` より下であることが要る。これは `b` での
同じ主張だが、その仮定は水準 `a` では使えても、帰納法が回っている水準では
使えない。
