...
`ω` 以下の数ではなく、この系自身が名前を付けられる任意の順序数になります。
...
項は主項 `ψ_a(b)` の有限和です。添字が項であることが、この拡張の中身そのものです。
...
`cmp` は主項のリストの辞書式順序で、真の前半が小さい方になります。主項どうしは
`(添字, 引数)` の辞書式で比べる。Buchholz の `(<1)`–`(<3)` で、添字を数から項に
一般化したものです。

`Order.lean` はこれが狭義の線形順序であることを証明します。この 3 つは
`Googology.Core.Morphism` の `OrdHom.injective` が要求するものそのものなので、
この系への翻訳は順序を保つだけで単射になります。

## 標準形

`ψ_a(b)` が標準形であるとは、`ψ_a` がまだ届く `b` の部分項が全部 `b` より真に
小さいことをいいます。和が標準形であるとは、主項が広義単調減少であることをいいます。
集める関数 `G` は Buchholz のものだが、内側の `ψ_c(d)` の添字 `c` 自身が項なので、
`d` と一緒に `c` も集める点が違う。

`isOT` は `Bool` を返す決定手続きなので、小さい場合は `#guard` が計算で確かめます。
...
`WF.lean` は、**全部の項の上の順序が整礎でない**ことを証明しています。

```
Ω > ψ_0(Ω) > ψ_0(ψ_0(Ω)) > ψ_0(ψ_0(ψ_0(Ω))) > ⋯
```

`cmp` が添字を先に比べ、`0 < ψ_0(0)` なので、これが無限に下がります。この列は 3 項目で
`OT` の外に出ます。だから `not_wellFounded_lt` は欠陥ではなく、標準形の条件が存在する
理由そのものです。

## 較正

出典は、巨大数研究 Wiki の P進大好きbot 氏による「拡張 Buchholz OCF に伴う順序数
表記」の記事です。以下は全部、その条項と 1 つずつ突き合わせました。
...
出典は、`o` の `OT` への制限が `C_0(Λ)` への順序**同型**だと述べています。単調で
...
したがって問題は**主項についての仮定 1 つ**に落ちました。和の側に内容はもう無い。

その半分の筋道。標準形は主項が広義単調減少である（`OT_tail_head_le`）ので、頭が
`p` 以下の標準形は「`p` の連続する塊」＋「頭が `p` より真に小さい項」の形になります。
`lead_lex` は、比較がこの表示を辞書式に読むことを言います。塊が長い方が大きく、塊が
同じ長さのときだけ残りが効きます。だから帰納は `Nat` の塊の長さで回り、長さを固定した
中では塊のあとの部分の可到達性で回る。`OT` の外の項は自動的に可到達です。
`OTLt y t` が `OT t` を要求するからです。

`acc_of_headLe` は全体仮定をまったく必要としません。境界の項の可到達性そのものが
帰納の軸になっています。
...
これが崩壊の議論です。ここで取る道は順序数の側です。`Ord.lean` が Maksudov の
条項から順序数の上の `ψ` を定義しており、予定は評価写像 `Term → Ordinal` を作って、
`OT` の上での単調性から `OrdHom.wf` で整礎性を引き戻すことです。

`Ord.lean` には濃度評価が入った。閉包を有限段の合併として表し、各段が濃度
`max #(Iio Ω_v) ℵ₀` 以下であることを濃度の計算で示し、その最大値がどの `v` でも
`ℵ_v` であることを示した——`Ω_0 = 1` の `v = 0` でも閉包は可算です。
`ℵ_v < ℵ_{v+1}` だから、閉包は `Ω_{v+1}` 未満の順序数を覆いきれない。したがって
`ψ_v(a) < Ω_{v+1}` です。

`Eval.lean` が `Term.val` を定義した。残っているのはその `OT` 上での単調性です。
すぐに出ない理由は記録しておく価値があります。`ψ_v` の引数についての弱い単調性では
足りません。`ψ_v(a) = ψ_v(a+1)` は実際に起きる——`a` が `C_v(a)` の中に届かないとき
ちょうどそうなります。狭義にするのが標準形の条件の役目です。4 段階は `Eval.lean` の
冒頭に書いてあります。構文的な道（Buchholz の集合 `W_u` と Bachmann 性質）を
取らなかったのは、拡張版の基本列が先に要るのに、確認できた出典が手元に無かったから
です。

### 最後の補題への道筋

Buchholz は補題 3.3 を `b ⊲_z a` という関係を経由して証明します。`b` が `a` より
...
変える。`Closure.lean` に語彙と 3.5 の両方があります。どちらも「2 つの項の狭義の間にある項の形」を決める分解補題に乗っています。
...
例外的に `sorry` を許しているわけではありません。ファイルに `sorry` も `axiom` もありません。

### 出典のあるもの・無いもの

上の `ψ` の定義は Maksudov のもので、Googology Wiki に載っています。それに対応する
表記系は P進大好きbot によるもので、ここの項・順序・`G`・`OT`・評価はその記事に
従っています。条項ごとの突き合わせは、上の**較正**の節にまとめてあります。同記事の基本列はまだ実装
していません。

## 名前について

`EBuchholz` でも `EBP` でもなく `ExBuchholz` にしました。mathlib は「拡張した X」を
`E` の接頭辞で綴ります——`ENat`、`EReal`、`ENNReal`、`EMetricSpace`——が、その読み方は
人名の前では壊れます。`EBuchholz` はイニシャル「E. Buchholz」と見分けがつきません。
`Ex` なら「拡張」以外には読めません。
よければ 1 行の変更で済みます。
