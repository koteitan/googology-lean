...
その上で `Trian_case4` が、1 つの主張から場合 4 を出す。これを `SubBound` と
呼ぶ。定義域が項で添字づけられた標準形 `X` 1 つについての主張で、`dom X` の
添字を `Z` と書くと次のようになる。
...
`sub_G_le` が `Z` での 3.6 を使ってこれを `Z[0]` まで運び、`tower_G_le` が
そこから塔の上まで運ぶ。同じ評価が各段 `W_i` について、`X₂[W_i]` と `X₂` の間の
どの `c` に対しても成り立つ。帰納法は段について回し、`X₂` での 3.6 と塔の単調性
を使う。
...
指標は `ψ_{Z[0]}(0)` でなければならず、`dom X` より下の任意の `W` では駄目で
ある。`X = ψ_{ω+1}(0)` では定義域が `X` 自身で `X[0] = 0` なので、空の `G_u(0)`
で抑えることになってしまう。
...
以下の全レベル、サイズ 4 以下の全候補 `c` で確認してある。
...
| `SubBound` そのもの | **未証明**。標準形 571 個について計算で確認 |
