...
`TowerBound` は `test/ExBuchholzCheck.lean` で計算により確認してある。サイズ 8
以下の可算な場合 4 の形 532 個について 5 段、サイズ 7 以下の標準形の場合 4 の形
158 個（可算とは限らない）について 4 段。そこには必要な事実を 2 つ切り出して
ある。1 つは `G_0(X₂) < X₂` で、標準形一般には成り立たない（`ψ_Ω(ε₀)` は標準形
だが `G_0(ε₀)` は `Ω` を含む）ものの、場合 4 の配置では成り立つ。もう 1 つは、
塔の添字 `Z[0]` に `G` が見るものが `X₂[W₀]` より下にあることである。
...
| `TowerBound` そのもの | **未証明**。場合 4 の形 532 + 158 個について計算で確認 |
