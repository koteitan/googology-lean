...
Taken from koteitan, 1y-wo-por, `ZeroY/Mountain/SumInverse.lean` (https://github.com/koteitan/1y-wo-por,
revision db86f2d13c2e18a8e2c2fae5e083d00fc4d48a78, Apache-2.0).
Ported to Lean 4.30.0 and Mathlib v4.30.0 for koteitan/googology-lean: module names in imports are prefixed with `Googology.Notation.Y.WellOrder.`.
Port change: in `sumRow_nextLayer` the last `omega` is replaced by `Nat.sub_add_cancel`.
-/
import Googology.Notation.Y.WellOrder.ZeroY.Decode
import Googology.Notation.Y.WellOrder.ZeroY.Mountain.Termination
...
theorem sumRow_nextLayer {layer : Layer}
  ...
  | some found =>
      ...
      simp only at *
      exact (Nat.sub_add_cancel (Nat.le_of_lt hLess)).symm
