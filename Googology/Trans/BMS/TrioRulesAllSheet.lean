...
/-!
# The merged rules against the sheet, and the fixes' own checks

`TrioRulesAll.lean` is rules 1–10 of [koteitan/trio](https://github.com/koteitan/trio)
with Fixes A–E and N in one builder.  This file re-runs, for it, every check of
`TrioRules2Sheet.lean`, `TrioRules3Sheet.lean` and `TrioRulesNonLastSheet.lean`,
and the guards of `TrioRules2.lean` and `TrioRules3.lean`, then looks for
interactions between Fix E and Fix N.

* **The sheet** ([`tools/omega_alpha_rows.tsv`](https://github.com/koteitan/trio/blob/main/tools/omega_alpha_rows.tsv)):
  all 831 `#guard`s of `TrioRules2Sheet.lean` (the 744 agreeing rows, the 22
  `S` rows, the 9 `L` rows under both labels, the 8 `N`/`R` rows, rows 3439, 3480 and
  4533) hold with `trioRuleMatrixOfAll` in place of `trioRuleMatrixOf2`.  One of them
  changes on purpose: the printed label of row 3552 gets Fix E's matrix, not
  rules 1–10's.
* On the 822 labels (813 rows, 9 corrected `L` labels) the merged rules give
  `TrioRules3`'s matrix everywhere, and `TrioRulesNonLast`'s and `TrioRules2`'s matrix
  everywhere except the printed label of row 3552.  They differ from rules 1–10
  (`TrioRules.lean`) on 24 labels: the 22 `S` rows, row 4533 (Fix A) and the printed
  label of row 3552 (Fix E).
* **The order checks**: the 783 chosen matrices (306,153 pairs) and the 784 with the
  printed label of row 3552 (306,936 pairs): 0 disagreements.
* **The fixes' own guards**: Fixes A–D on one row each (`TrioRules2.lean`), Fix E
  (`TrioRules3.lean`: the matrix, its place, its expansion), Fix N (the plan's case,
  the lifted copy, 27 BMS expansions, the nearby families with the same counts).
* **Interactions.**  On the 822 labels, the two families and the 71 nearby
  ordinals, no label is changed by both Fix E and Fix N, and the merged matrix is
  always that of the version whose fix applies.  They do meet on hand-made labels
  where a Fix E level is a leaf that Fix N treats (`Ω_{Ω_{Ω_Ω}} + Ω_p + …` with `p`
  the printed ordinal of row 3552), and where a Fix N subscript lies in a Fix E
  argument.  There the merged matrix differs from both single-fix matrices.  It is
  standard wherever the single-fix ones are (yaBMS `bms -s`, outside Lean), and
  it is in order with the 784 chosen matrices.
...
#guard trioRuleMatrixOfAll "(W5)" -- row 2433
...
#guard trioRuleMatrixOfAll "(W_2*3)" -- row 2725
...
#guard trioRuleMatrixOfAll "(W_3+W_2)" -- row 2798
...
#guard trioRuleMatrixOfAll "(W_3*2)" -- row 2799
...
#guard trioRuleMatrixOfAll "W_6" -- row 2810
...
#guard trioRuleMatrixOfAll "(W_w+W+2)" -- row 3368
...
#guard trioRuleMatrixOfAll "(W_w+W+w)" -- row 3369
...
#guard trioRuleMatrixOfAll "(W_w+W+w^2)" -- row 3370
...
#guard trioRuleMatrixOfAll "(W_w+W2)" -- row 3371
...
#guard trioRuleMatrixOfAll "(W_w+W*w)" -- row 3372
...
#guard trioRuleMatrixOfAll "(W_w+W^2)" -- row 3373
...
#guard trioRuleMatrixOfAll "(W_w+W^W)" -- row 3374
...
#guard trioRuleMatrixOfAll "(W_w+psi_1(W_w))" -- row 3375
...
#guard trioRuleMatrixOfAll "(W_w+W_2)" -- row 3376
...
#guard trioRuleMatrixOfAll "(W_w+W_3)" -- row 3377
...
#guard trioRuleMatrixOfAll "(W_w*W+w)" -- row 3479
...
#guard trioRuleMatrixOfAll "(W_w*W*w)" -- row 3483
...
#guard trioRuleMatrixOfAll "(W_w*W*w^2)" -- row 3484
...
#guard trioRuleMatrixOfAll "(W_w*W*w^w)" -- row 3485
...
#guard trioRuleMatrixOfAll "(W_w*W^2)" -- row 3486
...
#guard trioRuleMatrixOfAll "(W_w*W^3)" -- row 3487
...
#guard trioRuleMatrixOfAll "(W_w*W^w)" -- row 3488
...
#guard trioRuleMatrixOfAll "(W_w*W^W)" -- row 3489
...
#guard trioRuleMatrixOfAll "(W_w*psi_1(W_w))" -- row 3490
...
#guard trioRuleMatrixOfAll "(W_w*W_2)" -- row 3491
...
#guard trioRuleMatrixOfAll "(W_w*W_3)" -- row 3493
...
#guard trioRuleMatrixOfAll "(W_(w+2)+w2)" -- row 3780
...
#guard trioRuleMatrixOfAll "(W_(w+2)+w^2)" -- row 3781
...
#guard trioRuleMatrixOfAll "(W_(w+2)+W)" -- row 3782
...
#guard trioRuleMatrixOfAll "(W_(w+2)+W_w)" -- row 3783
...
#guard trioRuleMatrixOfAll "(W_(w+2)+W_(w+1))" -- row 3784
...
#guard trioRuleMatrixOfAll "(W_(w+2)*2)" -- row 3785
...
#guard trioRuleMatrixOfAll "(W_(w+2)*3)" -- row 3786
...
#guard trioRuleMatrixOfAll "(W_(w+3)*w)" -- row 3801
...
#guard trioRuleMatrixOfAll "W_(w+4)" -- row 3802
...
#guard trioRuleMatrixOfAll "(W_(w2+1)+w)" -- row 3915
...
#guard trioRuleMatrixOfAll "(W_(w2+1)+w^2)" -- row 3916
...
#guard trioRuleMatrixOfAll "(W_(w2+1)+W)" -- row 3917
...
#guard trioRuleMatrixOfAll "(W_(w2+1)*2)" -- row 3918
...
#guard trioRuleMatrixOfAll "(W_(w2+1)^2)" -- row 3921
...
#guard trioRuleMatrixOfAll "(W_(w3)+w)" -- row 3943
...
#guard trioRuleMatrixOfAll "(W_(w3)+w^2)" -- row 3944
...
#guard trioRuleMatrixOfAll "(W_(w3)+W)" -- row 3945
...
#guard trioRuleMatrixOfAll "(W_(w3)+W_(w2+1))" -- row 3946
...
#guard trioRuleMatrixOfAll "(W_(w3)*2)" -- row 3947
...
#guard trioRuleMatrixOfAll "(W_(w3)*w)" -- row 3948
...
#guard trioRuleMatrixOfAll "(W_(w3)^2)" -- row 3949
...
#guard trioRuleMatrixOfAll "psi_W_(w3+1)(W_(w3+1))" -- row 3950
...
#guard trioRuleMatrixOfAll "(W_(w^2*2)+W_(w^2))" -- row 4333
...
#guard trioRuleMatrixOfAll "(W_(w^2*2)+W_(w^2+1))" -- row 4334
...
#guard trioRuleMatrixOfAll "(W_(w^2*2)*2)" -- row 4335
...
#guard trioRuleMatrixOfAll "(W_W+2)" -- row 4452
...
#guard trioRuleMatrixOfAll "(W_W+w)" -- row 4453
...
#guard trioRuleMatrixOfAll "(W_W+w^2)" -- row 4454
...
#guard trioRuleMatrixOfAll "(W_W+W)" -- row 4455
...
#guard trioRuleMatrixOfAll "(W_W+W_2)" -- row 4456
...
#guard trioRuleMatrixOfAll "(W_W+W_w)" -- row 4457
...
#guard trioRuleMatrixOfAll "(W_W*2)" -- row 4458
...
#guard trioRuleMatrixOfAll "(W_W*3)" -- row 4459
...
#guard trioRuleMatrixOfAll "(W_W*W_2)" -- row 4462
...
#guard trioRuleMatrixOfAll "(W_W*W_w)" -- row 4463
...
#guard trioRuleMatrixOfAll "(W_W^2)" -- row 4464
...
#guard trioRuleMatrixOfAll "(W_W^W_W)" -- row 4465
...
#guard trioRuleMatrixOfAll "(W_(W+1)+w)" -- row 4486
...
#guard trioRuleMatrixOfAll "(W_(W+1)+W_2)" -- row 4487
...
#guard trioRuleMatrixOfAll "(W_(W+1)*2)" -- row 4489
...
#guard trioRuleMatrixOfAll "psi_W_(W+2)(W_(W+2))" -- row 4492
...
#guard trioRuleMatrixOfAll "W_(W+2)" -- row 4493
...
#guard trioRuleMatrixOfAll "W_(W+3)" -- row 4494
...
#guard trioRuleMatrixOfAll "W_(W+w^2)" -- row 4500
...
#guard trioRuleMatrixOfAll "W_(W+w^w)" -- row 4501
...
#guard trioRuleMatrixOfAll "W_(W2)" -- row 4502
...
#guard trioRuleMatrixOfAll "W_(W3)" -- row 4503
...
#guard trioRuleMatrixOfAll "(W_W_w+w)" -- row 4607
...
#guard trioRuleMatrixOfAll "(W_W_w+W)" -- row 4608
...
#guard trioRuleMatrixOfAll "(W_W_w+W_(w+1))" -- row 4610
...
#guard trioRuleMatrixOfAll "(W_W_w+W_(w2))" -- row 4611
...
#guard trioRuleMatrixOfAll "(W_W_w+W_W)" -- row 4612
...
#guard trioRuleMatrixOfAll "(W_W_w*w)" -- row 4614
...
#guard trioRuleMatrixOfAll "(W_W_w*W)" -- row 4615
...
#guard trioRuleMatrixOfAll "(W_W_w*W_(w+1))" -- row 4616
...
#guard trioRuleMatrixOfAll "(W_W_w*W_W)" -- row 4617
...
#guard trioRuleMatrixOfAll "psi_W_(W_w+1)(W_(W_w+1))" -- row 4619
...
#guard trioRuleMatrixOfAll "W_(W_w+w)" -- row 4622
...
#guard trioRuleMatrixOfAll "W_(W_w*2)" -- row 4623
...
#guard trioRuleMatrixOfAll "W_(W_w*3)" -- row 4624
...
#guard trioRuleMatrixOfAll "(W_W_(w+1)*2)" -- row 4641
...
#guard trioRuleMatrixOfAll "(W_W_(w+1)*w)" -- row 4642
...
#guard trioRuleMatrixOfAll "(W_W_(w+1)^2)" -- row 4643
...
#guard trioRuleMatrixOfAll "W_(W_(w+1)+1)" -- row 4644
...
#guard trioRuleMatrixOfAll "W_(W_(w+1)*2)" -- row 4645
...
#guard trioRuleMatrixOfAll "W_(W_(w+1)*w)" -- row 4646
...
#guard trioRuleMatrixOfAll "W_(W_(w+1)^2)" -- row 4647
...
#guard trioRuleMatrixOfAll "W_W_(w+2)" -- row 4648
...
#guard trioRuleMatrixOfAll "W_W_(w3)" -- row 4651
...
#guard trioRuleMatrixOfAll "W_W_(w^2)" -- row 4652
  = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1]]
#guard trioRuleMatrixOfAll "(W_W_(w^2)+1)" -- row 4665
...
#guard trioRuleMatrixOfAll "(W_W_(w^2)+W_(w^2+1))" -- row 4666
...
#guard trioRuleMatrixOfAll "(W_W_(w^2)*w)" -- row 4668
...
#guard trioRuleMatrixOfAll "W_(W_(w^2)+1)" -- row 4669
...
#guard trioRuleMatrixOfAll "W_(W_(w^2)+W)" -- row 4670
...
#guard trioRuleMatrixOfAll "W_(W_(w^2)*2)" -- row 4671
...
#guard trioRuleMatrixOfAll "(W_W_W+w)" -- row 4741
...
#guard trioRuleMatrixOfAll "(W_W_W+W_2)" -- row 4742
...
#guard trioRuleMatrixOfAll "(W_W_W+W_w)" -- row 4743
...
#guard trioRuleMatrixOfAll "(W_W_W*w)" -- row 4748
...
#guard trioRuleMatrixOfAll "(W_W_W*W_2)" -- row 4749
...
#guard trioRuleMatrixOfAll "psi_W_(W_W+1)(W_(W_W+1))" -- row 4754
...
#guard trioRuleMatrixOfAll "W_(W_W+1)" -- row 4755
...
#guard trioRuleMatrixOfAll "W_(W_W+w)" -- row 4756
...
#guard trioRuleMatrixOfAll "W_(W_W+W_2)" -- row 4757
...
#guard trioRuleMatrixOfAll "W_(W_W*2)" -- row 4758
...
#guard trioRuleMatrixOfAll "W_(W_W*w)" -- row 4759
...
#guard trioRuleMatrixOfAll "W_(W_W*W_2)" -- row 4760
...
#guard trioRuleMatrixOfAll "W_(W_W^2)" -- row 4761
...
#guard trioRuleMatrixOfAll "W_W_(W+1)" -- row 4763
...
#guard trioRuleMatrixOfAll "W_W_(W+w)" -- row 4764
...
#guard trioRuleMatrixOfAll "W_W_(W2)" -- row 4765
...
#guard trioRuleMatrixOfAll "W_W_(W*w)" -- row 4766
...
#guard trioRuleMatrixOfAll "W_W_(W^2)" -- row 4767
...
#guard trioRuleMatrixOfAll "W_W_(W^W)" -- row 4768
...
#guard trioRuleMatrixOfAll "W_W_W_2" -- row 4770
...
#guard trioRuleMatrixOfAll "W_W_W_3" -- row 4771
...
#guard trioRuleMatrixOfAll "W_W_W_w" -- row 4772
  = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1]]
#guard trioRuleMatrixOfAll "W_W_W_(w+1)" -- row 4773
...
#guard trioRuleMatrixOfAll "W_W_W_(w2)" -- row 4774
...
-- row 3492 (S-o)
#guard trioRuleMatrixOfAll "(W_w*W_2*w)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[5,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,1],[6,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,1],[7,2,0],[6,4,1]]
...
-- row 4488 (S-o)
#guard trioRuleMatrixOfAll "(W_(W+1)+W_W)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4490 (S-o)
#guard trioRuleMatrixOfAll "(W_(W+1)*w)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,1]]
...
-- row 4491 (S-o)
#guard trioRuleMatrixOfAll "(W_(W+1)^2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,1],[7,3,0]]
...
-- row 4497 (S-c)
#guard trioRuleMatrixOfAll "(W_(W+w)*w)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1]]
...
-- row 4508 (S-o)
#guard trioRuleMatrixOfAll "W_psi_1(W_2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[5,2,0]]
...
-- row 4609 (S-o)
#guard trioRuleMatrixOfAll "(W_W_w+W_w)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[6,3,0],[7,4,1],[8,4,1],[9,1,0],[1,1,1]]
...
-- row 4613 (S-o)
#guard trioRuleMatrixOfAll "(W_W_w*2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[6,3,0],[7,4,1],[8,4,1],[9,2,0],[3,2,1],[4,2,1],[5,1,0],[1,1,1]]
...
-- row 4618 (S-o)
#guard trioRuleMatrixOfAll "(W_W_w^2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[6,3,1],[7,2,0],[3,2,1],[4,2,1],[5,1,0],[1,1,1]]
...
-- row 4628 (S-o)
#guard trioRuleMatrixOfAll "W_psi_W_(w+1)(W_(w+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[6,2,0]]
...
-- row 4667 (S-c)
#guard trioRuleMatrixOfAll "(W_W_(w^2)*2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[6,3,0],[7,4,1],[8,4,1],[9,2,0],[3,2,1],[4,2,1],[5,1,0],[1,1,1],[2,1,1]]
...
-- row 4674 (S-o)
#guard trioRuleMatrixOfAll "W_(psi_W_(w^2+1)(W_(w^2+1)))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[6,2,0]]
...
-- row 4744 (S-c)
#guard trioRuleMatrixOfAll "(W_W_W+W_W)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4745 (S-c)
#guard trioRuleMatrixOfAll "(W_W_W+W_(W+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0]]
...
-- row 4746 (S-c)
#guard trioRuleMatrixOfAll "(W_W_W+W_W_2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0]]
...
-- row 4747 (S-c)
#guard trioRuleMatrixOfAll "(W_W_W*2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4750 (S-c)
#guard trioRuleMatrixOfAll "(W_W_W*W_W)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1],[8,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4751 (S-c)
#guard trioRuleMatrixOfAll "(W_W_W*W_(W+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1],[8,3,0]]
...
-- row 4752 (S-c)
#guard trioRuleMatrixOfAll "(W_W_W*W_W_2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1],[8,3,0],[4,3,1],[5,3,1],[6,2,0]]
...
-- row 4753 (S-c)
#guard trioRuleMatrixOfAll "(W_W_W^2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1],[8,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4762 (S-o)
#guard trioRuleMatrixOfAll "W_psi_W_(W+1)(W_(W+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[7,3,0]]
...
-- row 4769 (S-o)
#guard trioRuleMatrixOfAll "W_W_psi_1(W_2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[5,2,0]]
...
#guard trioRuleMatrixOfAll "(w^5*2)" = some [[0,0,0],[1,1,1],[2,1,1],[2,1,1],[2,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[4,2,1],[4,2,1],[4,2,1]]
#guard trioRuleMatrixOfAll "(w^5*3)" = trioRuleMatrixOf "(w^5*3)"
-- row 2131, printed `(w^w+w2)`, corrected `(w^w+2)`
#guard trioRuleMatrixOfAll "(w^w+2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,0,0],[2,1,0],[3,2,0],[4,3,0]]
#guard trioRuleMatrixOfAll "(w^w+w2)" = trioRuleMatrixOf "(w^w+w2)"
-- row 2532, printed `(W^(W+1))`, corrected `(W^(w+1))`
#guard trioRuleMatrixOfAll "(W^(w+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,1,0],[4,0,0],[3,2,1],[4,1,0]]
#guard trioRuleMatrixOfAll "(W^(W+1))" = trioRuleMatrixOf "(W^(W+1))"
-- row 2723, printed `(W_2*2+W)`, corrected `(W_2*2+w^2)`
#guard trioRuleMatrixOfAll "(W_2*2+w^2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,0],[3,3,1],[4,3,1],[5,2,0],[4,3,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,1],[8,5,1]]
#guard trioRuleMatrixOfAll "(W_2*2+W)" = trioRuleMatrixOf "(W_2*2+W)"
-- row 3452, printed `(W_w*w+W_w+10)`, corrected `(W_w*w+W_w+1)`
#guard trioRuleMatrixOfAll "(W_w*w+W_w+1)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[4,2,0],[5,3,1],[6,3,1],[7,1,0],[6,3,0],[7,4,0]]
#guard trioRuleMatrixOfAll "(W_w*w+W_w+10)" = trioRuleMatrixOf "(W_w*w+W_w+10)"
-- row 3552, printed `psi_W_(W+1)(W_psi_W_(w+1)(W_(w+1)))`, corrected `psi_W_(w+1)(W_psi_W_(w+1)(W_(w+1)))`
#guard trioRuleMatrixOfAll "psi_W_(w+1)(W_psi_W_(w+1)(W_(w+1)))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[6,2,1],[7,2,1],[8,1,0],[9,2,0]]
-- Fix E: the printed label now gets `TrioRules3`'s matrix, not rules 1–10's.
#guard trioRuleMatrixOfAll printed3552 = trioRuleMatrixOf3 printed3552
#guard trioRuleMatrixOfAll printed3552 ≠ trioRuleMatrixOf printed3552
-- row 3777, printed `(W_(w+2)+w)`, corrected `(W_(w+2)+2)`
#guard trioRuleMatrixOfAll "(W_(w+2)+2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,0],[8,6,0]]
#guard trioRuleMatrixOfAll "(W_(w+2)+w)" = trioRuleMatrixOf "(W_(w+2)+w)"
-- row 4369, printed `(W_(w^3*W))`, corrected `(W_(w^3)*W)`
#guard trioRuleMatrixOfAll "(W_(w^3)*W)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[5,1,0]]
#guard trioRuleMatrixOfAll "(W_(w^3*W))" = trioRuleMatrixOf "(W_(w^3*W))"
-- row 4384, printed `W_(w^2*2)`, corrected `W_(w^w*2)`
#guard trioRuleMatrixOfAll "W_(w^w*2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,0,0],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,0,0]]
#guard trioRuleMatrixOfAll "W_(w^2*2)" = trioRuleMatrixOf "W_(w^2*2)"

/-! ### The 8 rows where the rules are right and the sheet has another matrix -/

-- row 2133 (R)
#guard trioRuleMatrixOfAll "(w^w+w2)" = trioRuleMatrixOf "(w^w+w2)"
#guard trioRuleMatrixOfAll "(w^w+w2)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,0,0],[2,1,0],[3,2,1],[3,2,0],[4,3,1]]
-- row 3453 (N)
#guard trioRuleMatrixOfAll "(W_w*w+W_w*2)" = trioRuleMatrixOf "(W_w*w+W_w*2)"
#guard trioRuleMatrixOfAll "(W_w*w+W_w*2)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[4,2,0],[5,3,1],[6,3,1],[7,1,0],[6,3,0],[7,4,1],[8,4,1],[9,1,0]]
-- row 3551 (R (weak))
#guard trioRuleMatrixOfAll "psi_W_(w+1)(W_W_w)" = trioRuleMatrixOf "psi_W_(w+1)(W_W_w)"
#guard trioRuleMatrixOfAll "psi_W_(w+1)(W_W_w)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[6,2,1],[7,2,1],[8,1,0],[1,1,1]]
-- row 3706 (N)
#guard trioRuleMatrixOfAll "(W_(w+1)+W_w^2)" = trioRuleMatrixOf "(W_(w+1)+W_w^2)"
#guard trioRuleMatrixOfAll "(W_(w+1)+W_w^2)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,1,0],[8,1,0],[1,1,1]]
-- row 3709 (N)
#guard trioRuleMatrixOfAll "(W_(w+1)*2+W)" = trioRuleMatrixOf "(W_(w+1)*2+W)"
#guard trioRuleMatrixOfAll "(W_(w+1)*2+W)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,2,0],[7,4,0],[8,5,1]]
-- row 3923 (R)
#guard trioRuleMatrixOfAll "psi_W_(w2+2)(W_(w2+2))" = trioRuleMatrixOf "psi_W_(w2+2)(W_(w2+2))"
#guard trioRuleMatrixOfAll "psi_W_(w2+2)(W_(w2+2))" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,0],[5,3,1],[6,3,1],[7,3,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[9,3,0]]
-- row 4303 (N)
#guard trioRuleMatrixOfAll "(W_(w^2+w)^W_(w^2+w))" = trioRuleMatrixOf "(W_(w^2+w)^W_(w^2+w))"
#guard trioRuleMatrixOfAll "(W_(w^2+w)^W_(w^2+w))" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[8,2,0],[3,2,1]]
-- row 4496 (N)
#guard trioRuleMatrixOfAll "(W_(W+w)+1)" = trioRuleMatrixOf "(W_(W+w)+1)"
#guard trioRuleMatrixOfAll "(W_(W+w)+1)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1]]

/-! ### Rows 3439 and 3480: unchanged -/

-- row 3439 (X)
#guard trioRuleMatrixOfAll "(W_w*w0*W_W_w+W_(W_w*w))" = trioRuleMatrixOf "(W_w*w0*W_W_w+W_(W_w*w))"
#guard trioRuleMatrixOfAll "(W_w*w0*W_W_w+W_(W_w*w))" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[4,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,1,0],[6,3,1],[6,2,0],[5,3,1],[6,3,1],[7,1,0],[6,3,1]]
-- row 3480 (O)
#guard trioRuleMatrixOfAll "(W_w*W+W_3)" = trioRuleMatrixOf "(W_w*W+W_3)"
#guard trioRuleMatrixOfAll "(W_w*W+W_3)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[5,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,1],[6,1,0],[5,3,0],[6,4,1],[7,4,1],[8,1,0],[7,4,1]]
...
#guard trioRuleMatrixOfAll "W_psi_2(W_3)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,2,0],[6,3,0]]
...
def chosenDataAll : Array (Od × List (List Nat)) :=
  (chosen.map fun s => ((parse s).getD [], allOf s)).toArray

#guard chosenDataAll.size = 783
#guard chosenDataAll.all fun p => !p.1.isEmpty && !p.2.isEmpty
...
#guard allLabels.all fun s => trioRuleMatrixOfAll s == trioRuleMatrixOf3 s
#guard (allLabels.filter (· != printed3552)).all fun s => trioRuleMatrixOfAll s == trioRuleMatrixOf2 s
#guard trioRuleMatrixOfAll printed3552 != trioRuleMatrixOf2 printed3552

/-- The 783 labels and the printed label of row 3552, with the merged matrices. -/
def chosenDataAll3 : Array (Od × List (List Nat)) :=
  ((chosen ++ [printed3552]).map fun s => ((parse s).getD [], allOf s)).toArray

#guard chosenDataAll3.size = 784
#guard chosenDataAll3.all fun p => !p.1.isEmpty && !p.2.isEmpty
#guard orderDisagreements chosenDataAll3 = 0

#guard allOf printed3552 = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[7,3,1],[8,3,1],[9,2,0],[2,2,1],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[7,3,0]]
#guard allOf printed3552 =
  allOf "psi_W_(W+1)(W_W_2)" ++ liftRows ((allOf "psi_W_(w+1)(W_(w+1))").drop 4)
#guard allOf "psi_W_(W+1)(W_W_2)" < allOf "psi_W_(W+1)(W_W_w)"
#guard allOf "psi_W_(W+1)(W_W_w)" < allOf printed3552
#guard allOf printed3552 < allOf "W_(W+1)"
#guard (List.range 4).all fun n =>
  expandRL 3 n (allOf printed3552) =
    allOf "psi_W_(W+1)(W_W_2)" ++
      liftRows ((expandRL 3 n (allOf "psi_W_(w+1)(W_(w+1))")).drop 4)
#guard allOf "psi_W_(W+1)(W_W_w)" = allOf "psi_W_(W+1)(W_W_2)" ++ [[2,2,1]]
#guard expandRL 3 1 (allOf "psi_W_(W+1)(W_W_w)") =
  allOf "psi_W_(W+1)(W_W_2)" ++
    [[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[8,4,1],[9,4,1],[10,3,0]]
#guard expandRL 3 2 (allOf "W_(W+1)") = allOf "psi_W_(W+1)(W_W_2)" ++ [[10,3,1],[11,3,1]]
#guard allOf "psi_W_(W+1)(W_psi_W_(W+1)(W_(W+1)))" = allOf "psi_W_(W+1)(W_W_2)" ++ [[10,3,0]]
#guard expandRL 3 2 (allOf "psi_W_(W+1)(W_psi_W_(W+1)(W_(W+1)))") =
  allOf "psi_W_(W+1)(W_W_2)" ++ [[10,2,0],[11,2,0]]
#guard allOf printed3552 < allOf "psi_W_(W+1)(W_psi_W_(W+1)(W_(W+1)))"
#guard allOf "psi_W_(w+1)(W_W)" = row3550
#guard allOf "psi_W_(w+1)(W_W_w)" = row3550 ++ [[6,2,1]]
...
#guard (sheetLabels.filter (· != printed3552)).all fun s => trioRuleMatrixOfAll s == trioRuleMatrixOfN s
#guard sheetLabels.all fun s => trioRuleMatrixOfAll s == trioRuleMatrixOf3 s

-- Fix N on the plan's case, the lifted copy, and BMS expansion.
#guard allOf "(W_W_W+W_W_2+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,4,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,4,0],[8,5,0],[9,6,1],[10,6,1],[11,4,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,0]]
#guard allOf "(W_W_W+W_(W+1)+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,0],[5,4,1],[6,4,1],[7,4,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,1],[8,5,1],[9,4,0],[8,5,0],[9,6,1],[10,6,1],[11,3,0],[10,6,0],[11,7,0]]
#guard allOf "(W_W_W*2+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[9,5,0],[10,6,0]]
#guard allOf "(W_W+W_2+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,2,0],[8,5,0],[9,6,0]]
#guard allOf "(W_W*2+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,2,0],[7,4,0],[8,5,0]]
#guard allOf "(W_W_W+W_W_2*2)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,4,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,4,0],[8,5,0],[9,6,1],[10,6,1],[11,4,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,1],[8,5,1],[9,3,0],[5,4,1],[6,4,1],[7,2,0]]
#guard allOf "(W_W_W+W_W_2*w)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,4,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,4,0],[8,5,0],[9,6,1],[10,6,1],[11,4,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,1],[8,5,1],[9,3,0],[8,5,1]]
...
#guard cmpMat (allOf "(W_W_W+W_W_2)") (allOf "(W_W_W+W_W_2+1)") = .lt
#guard cmpMat (allOf "(W_W_W+W_W_2+1)") (allOf "(W_W_W+W_W_2*2)") = .lt
#guard cmpMat (allOf "(W_W_W+W_W_2*2)") (allOf "(W_W_W*2)") = .lt
#guard cmpMat (allOf "(W_W_W+W_(W+1)+1)") (allOf "(W_W_W+W_W_2)") = .lt
#guard cmpMat (allOf "(W_W_W*2)") (allOf "(W_W_W*2+1)") = .lt
#guard cmpMat (allOf "(W_W+W_2+1)") (allOf "(W_W*2)") = .lt
#guard cmpMat (allOf "(W_W+W_2+1)") (allOf "(W_W+W_w)") = .lt
#guard expandRL 3 1 (allOf "(W_W_W+W_W_w)") =
  toRows (MAll ((parse "(W_W_W+W_W_2)").getD []) ++ bliftUp (MAll ((parse "(W_W_W+W_W_2)").getD [])) 10)
#guard expandRL 3 1 (allOf "(W_W_W+W_w)") =
  toRows (MAll ((parse "(W_W_W+W_2)").getD []) ++ bliftUp (MAll ((parse "(W_W_W+W_2)").getD [])) 10)
#guard expandRL 3 1 (allOf "(W_W+W_w)") =
  toRows (MAll ((parse "(W_W+W_2)").getD []) ++ bliftUp (MAll ((parse "(W_W+W_2)").getD [])) 7)
#guard expandRL 3 1 (allOf "(W_W*w)") = allOf "(W_W+W_2)"
#guard expandRL 3 2 (allOf "(W_W*w)") = allOf "(W_W*2+W_2)"
#guard expandRL 3 1 (allOf "(W_W_W*w)") = allOf "(W_W_W+W_(W+1))"
#guard expandRL 3 2 (allOf "(W_W_W*w)") = allOf "(W_W_W*2+W_(W+1))"
#guard expandRL 3 1 (allOf "(W_W_W^2*w)") = allOf "(W_W_W^2+W_W_W*W_(W+1))"
#guard expandRL 3 2 (allOf "(W_W_W^2*w)") = allOf "(W_W_W^2*2+W_W_W*W_(W+1))"
#guard expandRL 3 1 (allOf "(W_W_W+W_2*w)") = allOf "(W_W_W+W_2*2)"
#guard expandRL 3 2 (allOf "(W_W_W+W_2*w)") = allOf "(W_W_W+W_2*3)"
#guard expandRL 3 1 (allOf "(W_W_W+W_(W+1)*w)") = allOf "(W_W_W+W_(W+1)*2)"
#guard expandRL 3 2 (allOf "(W_W_W+W_(W+1)*w)") = allOf "(W_W_W+W_(W+1)*3)"
#guard expandRL 3 1 (allOf "(W_W_W+W_W_2*w)") = allOf "(W_W_W+W_W_2*2+W_(W+1))"
#guard expandRL 3 2 (allOf "(W_W_W+W_W_2*w)") = allOf "(W_W_W+W_W_2*3+W_(W+1))"
#guard expandRL 3 1 (allOf "(W_W_W*W_2*w)") = allOf "(W_W_W*W_2*2)"
#guard expandRL 3 2 (allOf "(W_W_W*W_2*w)") = allOf "(W_W_W*W_2*3)"
#guard expandRL 3 1 (allOf "(W_W_W*W_(W+1)*w)") = allOf "(W_W_W*W_(W+1)*2)"
#guard expandRL 3 2 (allOf "(W_W_W*W_(W+1)*w)") = allOf "(W_W_W*W_(W+1)*3)"
#guard expandRL 3 1 (allOf "(W_W_W*W_W_2*w)") = allOf "(W_W_W*W_W_2*2+W_W_W*W_(W+1))"
#guard expandRL 3 1 (allOf "(W_W_W+W_W_2+W_2*w)") = allOf "(W_W_W+W_W_2+W_2*2)"
#guard expandRL 3 2 (allOf "(W_W_W+W_W_2+W_2*w)") = allOf "(W_W_W+W_W_2+W_2*3)"
#guard expandRL 3 1 (allOf "(W_W_W+W_(W+1)+W_2*w)") = allOf "(W_W_W+W_(W+1)+W_2*2)"
#guard expandRL 3 2 (allOf "(W_W_W+W_(W+1)+W_2*w)") = allOf "(W_W_W+W_(W+1)+W_2*3)"
#guard expandRL 3 1 (allOf "(W_W_W+W_W_2+W_(W+1)*w)") = allOf "(W_W_W+W_W_2+W_(W+1)*2)"
#guard expandRL 3 2 (allOf "(W_W_W+W_W_2+W_(W+1)*w)") = allOf "(W_W_W+W_W_2+W_(W+1)*3)"
#guard expandRL 3 1 (allOf "(W_W+W_2*w)") = allOf "(W_W+W_2*2)"
#guard expandRL 3 2 (allOf "(W_W+W_2*w)") = allOf "(W_W+W_2*3)"
#guard expandRL 3 1 (allOf "(W_W+W_2+W*w)") = allOf "(W_W+W_2+W*2)"
#guard expandRL 3 2 (allOf "(W_W+W_2+W*w)") = allOf "(W_W+W_2+W*3)"

-- The nearby families: the same counts as with Fix N alone.
#guard pairDisagreements (withM allOf familyW) = 1 ∧ collisions (withM allOf familyW) = 1
#guard pairDisagreements (withM allOf family1) = 0 ∧ collisions (withM allOf family1) = 0
#guard pairDisagreements (withM allOf probes) = 25
#guard crossDisagreements (withM allOf probes) chosenDataAll = 20
#guard crossDisagreements (withM allOf probes) chosenDataAll3 = 20
...
#guard (sheetLabels.filter fun s => trioRuleMatrixOfAll s != trioRuleMatrixOf s).length = 24
#guard (sheetLabels.filter fun s => trioRuleMatrixOf2 s != trioRuleMatrixOf s).length = 23
#guard (sheetLabels.filter fun s => trioRuleMatrixOfAll s != trioRuleMatrixOf s).all fun s =>
...
def meet (s : String) : Bool × Bool × Bool × Bool :=
  (ruleOf3 s != ruleOf2 s, nOf s != ruleOf2 s, allOf s == ruleOf3 s, allOf s == nOf s)
...
def MfPrev : Od → Cols := MfuelAll (fuel - 1)
...
def stepDiffers (s : String) : Bool :=
  let a := (parse s).getD []
  MstepAll MfPrev a != Mstep MfPrev a

/-- `MAll` is one merged step on `MfPrev`. -/
theorem MAll_eq_step : MAll = MstepAll MfPrev := rfl
...
#guard (corpus.filter fun s => branch ((parse s).getD []) == "psi").all fun s =>
  ...
  | some uX => stepDiffers s == (fixAFires MfPrev uX.1 || fixEFires MfPrev uX.1 uX.2)
  | none => false
#guard (corpus.filter fun s => branch ((parse s).getD []) == "omega").all fun s =>
  match lvlO ((parse s).getD []) with
  | some v => stepDiffers s == fixAFires MfPrev v
  | none => false
