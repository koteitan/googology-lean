...
See `TrioFixStripSheet.lean` for the summary.  Part 9 (the checks of `TrioFixOfTermSheet.lean`), first group: the 42
terms with a sheet row, `ψ_0(Ω+1) = ε₀·ω` under BMS expansion, and `smallFrag 6`
(610 terms: 0 disagreements, 0 collisions).
...
#guard sheetPairs.all fun p => trioMatrixLSt p.1 == stOf p.2
#guard sheetPairs.all fun p => trioMatrixLSt p.1 == trioMatrixLFix p.1
...
#guard crossDisagreements (pairData trioMatrixLSt) chosenDataSt3 = 0

/-! ### `ψ_0(Ω+1) = ε₀·ω` and BMS expansion -/

#guard trioMatrixLSt (pOm t1) = stOf "(psi(W)*w)"
#guard trioMatrixLSt (pOm t1) = [[0,0,0],[1,1,1],[2,1,1],[3,0,0],[4,1,0],[2,1,1]]
#guard (List.range 4).all fun n =>
  expandRL 3 n (trioMatrixLSt (pOm t1)) =
    trioMatrixLSt (Googology.Notation.ExBuchholz.Term.repeatPrin nil tW (n + 1))
...
#guard (badPairs (smallFrag 6) trioMatrixLSt).length = 0
#guard collide (smallFrag 6) trioMatrixLSt = 0
