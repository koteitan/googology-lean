...
/-!
# The patch `TrioFixMulNormalize` against the sheet, with every check of `TrioRulesAllSheet`

`TrioFixMulNormalize.lean` is `TrioRulesAll.lean` (rules 1–10 of
[koteitan/trio](https://github.com/koteitan/trio) with Fixes A–E and N) with
`ω^atom = atom` in `mul` and `power` (so in the label reader) and a pattern match in
place of `==` in the step.  This file re-runs Parts 1–7 of `TrioRulesAllSheet.lean`
for the patched rules (`trioRuleMatrixOfAll`, `allOf`, `parse`, `MstepAll`,
`MfuelAll` replaced by `trioRuleMatrixOfFix`, `fixOf`, `parseM`, `MstepFix`,
`MfuelFix`), then adds the patch's own guards (Part 8).

* **The sheet**
  ([`tools/omega_alpha_rows.tsv`](https://github.com/koteitan/trio/blob/main/tools/omega_alpha_rows.tsv)):
  all 831 guards hold, with one intended change: the printed label of row 4369,
  `W_(w^3*W)` (`Ω_{ω^3·Ω}`, the sheet's label is a typo for `Ω_{ω^3}·Ω`), now gets
  `M(Ω_Ω)`, since `Ω_{ω^3·Ω} = Ω_Ω` and `cmpOrd` says so.  The corrected label keeps
  its matrix.
* So the checks that compare with the single-fix versions exclude that label, and
  the counts move by one: `interactionsF sheetLabels = [printed4369]`, and the
  patched rules differ from rules 1–10 on 25 sheet labels (24 before).
* **The order checks**: the 783 chosen matrices, the 784 with the printed label of
  row 3552, and the 785 with both printed labels: 0 disagreements (the merged rules
  without the patch: 1 on the 785).
* **Part 7** (one step against rules 1–10, by branch): the same counts as before,
  `(countable 171 0) (psi 32 6) (omega 152 7) (sum 870 257)`.
* **Part 8**: over all 1,209 distinct labels, `parseM` changes only the printed label
  of row 4369, every `parseM` output is a normal form, and only that label changes
  its matrix.  On 22 hand-made labels with an `ω^atom` exponent, the patched matrix
  is the matrix of the same ordinal written in normal form, all 22 are standard
  (yaBMS `bms -s`, outside Lean), and they are in order against the 784 chosen
  matrices (unpatched: 17 of 22 differ, 5 are not standard, 62 order disagreements).
...
/-- Labels with their matrices by `f`, the labels read by `parseM`. -/
def withMF (f : String → List (List Nat)) (ls : List String) : Array (Od × List (List Nat)) :=
  (ls.map fun s => ((parseM s).getD [], f s)).toArray

/-- The printed label of row 4369, `Ω_{ω^3·Ω}` (the corrected label is `Ω_{ω^3}·Ω`). -/
def printed4369 : String := "(W_(w^3*W))"
...
-- This patch: the printed label now reads `Ω_{ω^3·Ω} = Ω_Ω` and gets `M(Ω_Ω)`.
#guard trioRuleMatrixOfFix "(W_(w^3*W))" = trioRuleMatrixOfFix "W_W"
...
def chosenDataFix : Array (Od × List (List Nat)) :=
  (chosen.map fun s => ((parseM s).getD [], fixOf s)).toArray
...
-- This patch: all but the printed label of row 4369 (Part 8).
#guard (allLabels.filter (· != printed4369)).all fun s => trioRuleMatrixOfFix s == trioRuleMatrixOf3 s
#guard (allLabels.filter fun s => s != printed3552 && s != printed4369).all fun s =>
  trioRuleMatrixOfFix s == trioRuleMatrixOf2 s
...
def chosenDataFix3 : Array (Od × List (List Nat)) :=
  ((chosen ++ [printed3552]).map fun s => ((parseM s).getD [], fixOf s)).toArray
...
-- This patch: all but the printed label of row 4369 (Part 8).
#guard (sheetLabels.filter fun s => s != printed3552 && s != printed4369).all fun s =>
  trioRuleMatrixOfFix s == trioRuleMatrixOfN s
#guard (sheetLabels.filter (· != printed4369)).all fun s => trioRuleMatrixOfFix s == trioRuleMatrixOf3 s
...
#guard expandRL 3 1 (fixOf "(W_W_W+W_W_w)") =
  toRows (MFix ((parseM "(W_W_W+W_W_2)").getD []) ++ bliftUp (MFix ((parseM "(W_W_W+W_W_2)").getD [])) 10)
#guard expandRL 3 1 (fixOf "(W_W_W+W_w)") =
  toRows (MFix ((parseM "(W_W_W+W_2)").getD []) ++ bliftUp (MFix ((parseM "(W_W_W+W_2)").getD [])) 10)
#guard expandRL 3 1 (fixOf "(W_W+W_w)") =
  toRows (MFix ((parseM "(W_W+W_2)").getD []) ++ bliftUp (MFix ((parseM "(W_W+W_2)").getD [])) 7)
...
#guard pairDisagreements (withMF fixOf familyW) = 1 ∧ collisions (withMF fixOf familyW) = 1
#guard pairDisagreements (withMF fixOf family1) = 0 ∧ collisions (withMF fixOf family1) = 0
#guard pairDisagreements (withMF fixOf probes) = 25
#guard crossDisagreements (withMF fixOf probes) chosenDataFix = 20
#guard crossDisagreements (withMF fixOf probes) chosenDataFix3 = 20
...
-- This patch: the printed label of row 4369 is read differently from all three versions.
#guard interactionsF sheetLabels = [printed4369]
#guard interactionsF familyW = []
#guard interactionsF family1 = []
#guard interactionsF probes = []

-- Against rules 1–10: 25 labels differ (22 `S` rows, row 4533, printed rows 3552 and 4369);
-- `TrioRules2` differs on 23 of them.
#guard (sheetLabels.filter fun s => trioRuleMatrixOfFix s != trioRuleMatrixOf s).length = 25
...
#guard (sheetLabels.filter fun s => trioRuleMatrixOfFix s != trioRuleMatrixOf s).all fun s =>
  s == printed3552 || s == printed4369 || trioRuleMatrixOf2 s != trioRuleMatrixOf s
...
#guard probesEN.all fun s => (parseM s).isSome
...
#guard interactionsF probesEN = [
  ...
  "psi_W_(W+1)(W_psi_W_(w+1)(W_(W_W+W_2+1)))",
  "psi_W_(W+1)(W_psi_W_(w+1)(W_(W_W_W+W_(W+1)+1)))"]
#guard (interactionsF probesEN).all fun s => meetF s == (true, true, false, false)
-- On the other 12, only one fix acts and the merged matrix is that version's.
#guard (probesEN.filter fun s => !(interactionsF probesEN).contains s).all fun s =>
  let m := meetF s
...
-- collisions).
#guard crossDisagreements (withMF fixOf probesEN) chosenDataFix3 = 0
#guard crossDisagreements (withM nOf probesEN)
  (((chosen ++ [printed3552]).map fun s => ((parseM s).getD [], nOf s)).toArray) = 5
#guard pairDisagreements (withMF fixOf probesEN) = 3 ∧ collisions (withMF fixOf probesEN) = 3
...
def corpusF : List String := sheetLabels ++ familyW ++ family1 ++ probes ++ probesEN

/-- The merged builder one step of fuel down. -/
def MfPrevF : Od → Cols := MfuelFix (fuel - 1)

/-- The branch of the builder that `α` takes. -/
def branchF (a : Od) : String :=
  match lvlO a with
  | none => "countable"
  | some _ => match isPsiLevel a with
    | some _ => "psi"
    | none => if isOmegaForm a then "omega" else "sum"
...
#guard (["countable", "psi", "omega", "sum"].map fun k =>
    let ls := corpusF.filter fun s => branchF ((parseM s).getD []) == k
    (k, ls.length, (ls.filter stepDiffersF).length)) =
  [("countable", 171, 0), ("psi", 32, 6), ("omega", 152, 7), ("sum", 870, 257)]
#guard (corpusF.filter fun s => branchF ((parseM s).getD []) == "psi").all fun s =>
  match isPsiLevel ((parseM s).getD []) with
  | some uX => stepDiffersF s == (fixAFires MfPrevF uX.1 || fixEFires MfPrevF uX.1 uX.2)
  | none => false
#guard (corpusF.filter fun s => branchF ((parseM s).getD []) == "omega").all fun s =>
  match lvlO ((parseM s).getD []) with
  | some v => stepDiffersF s == fixAFires MfPrevF v
  | none => false

/-! ## Part 8: this patch's own guards (`ω^atom = atom`)

`everyLabel`: every label of this file and of the order checks (1,209 distinct).
The reader `parseM` changes exactly one of them, the printed label of row 4369
`Ω_{ω^3·Ω}`; it now reads as `Ω_Ω` (which `cmpOrd` says it is), and gets `M(Ω_Ω)`.
Nothing else changes.  `normalF` checks that no exponent is written `ω^atom`
(`.o [(atom, 1)]`), at any depth. -/

/-- Every label used in this file, without repeats. -/
def everyLabel : List String := (corpusF ++ chosen ++ allLabels).eraseDups

/-- No exponent `.o [(x, 1)]` with `x` an atom, at any depth (fuel `n`). -/
def normalF : Nat → Od → Bool
  | 0, _ => true
  | n + 1, a => a.all fun ec => match ec.1 with
      | .o b => (match b with | [(x, 1)] => !x.isAtom | _ => true) && normalF n b
      | .W v => normalF n v
      | .psi v X => normalF n v && normalF n X

#guard everyLabel.length = 1209
-- The reader: one label changes, and only it was not a normal form.
#guard everyLabel.filter (fun s => parseM s != parse s) = [printed4369]
#guard everyLabel.all fun s => normalF fuel ((parseM s).getD [])
#guard everyLabel.filter (fun s => !normalF fuel ((parse s).getD [])) = [printed4369]
#guard parseM printed4369 == parse "W_W"
#guard cmpOrd ((parse printed4369).getD []) ((parse "W_W").getD []) = .eq
-- The matrices: one label changes, to `M(Ω_Ω)`.
#guard everyLabel.filter (fun s => trioRuleMatrixOfFix s != trioRuleMatrixOfAll s) = [printed4369]
#guard fixOf printed4369 = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0]]
#guard fixOf printed4369 = allOf "W_W"
#guard allOf printed4369 = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0]]
-- The order check with both printed labels (785 labels): 0 disagreements; the
-- merged rules without this patch: 1 (the printed label of row 4369 against `Ω_Ω`).
#guard orderDisagreements (withMF fixOf (chosen ++ [printed3552, printed4369])) = 0
#guard orderDisagreements (withM allOf (chosen ++ [printed3552, printed4369])) = 1

/-- Hand-made labels with an `ω^atom` exponent (via `*` or `^`), each with the same
ordinal written in normal form. -/
def pairsNorm : List (String × String) := [
  ("(W_(w^3*W))", "W_W"), ("W_(w*W)", "W_W"), ("W_(2*W)", "W_W"), ("W_(1*W)", "W_W"),
  ("W_(w^W)", "W_W"), ("(w^W)", "W"), ("(w*W)", "W"), ("(W_(w^3*W)+1)", "(W_W+1)"),
  ("W_(w^3*W_2)", "W_W_2"), ("W_(w^3*W+1)", "W_(W+1)"),
  ("W_(w*psi_W_(w+1)(W_(w+1)))", "W_psi_W_(w+1)(W_(w+1))"), ("(W_(w^W)*2)", "(W_W*2)"),
  ("psi_W_(w^W+1)(W_(w^W+1))", "psi_W_(W+1)(W_(W+1))"), ("W_W_(w*W)", "W_W_W"),
  ("(W_(w^W)+W_(w^W_2))", "(W_W+W_W_2)"), ("(W_2*w^W)", "(W_2*W)"), ("W_(w^(W+1))", "W_(W*w)"),
  ("psi_W_(W+1)(W_(w*W_2))", "psi_W_(W+1)(W_W_2)"), ("W_(W_(w^W)+1)", "W_(W_W+1)"),
  ("(W_(w^W)^2)", "(W_W^2)"), ("(W_w*w^W)", "(W_w*W)"), ("W_(w*W_w)", "W_W_w")]

-- `cmpOrd` calls each pair equal; `parseM` reads the left label as the right one
-- (and in normal form), `parse` never does.
#guard pairsNorm.all fun p => cmpOrd ((parse p.1).getD []) ((parse p.2).getD []) == .eq
#guard pairsNorm.all fun p => parseM p.1 == parse p.2 && normalF fuel ((parseM p.1).getD [])
#guard pairsNorm.all fun p => parse p.1 != parse p.2
-- So the patched matrix of the left label is the merged matrix of the right one.  The
-- merged rules without the patch give another matrix on 17 of the 22.  The patched
-- matrices are all standard (yaBMS `bms -s`, outside Lean); 5 of the unpatched are not
-- (`(W_(w^3*W)+1)`, `W_(w*psi_W_(w+1)(W_(w+1)))`, `(W_(w^W)*2)`, `W_(W_(w^W)+1)`,
-- `(W_(w^W)^2)`).
#guard pairsNorm.all fun p => fixOf p.1 == allOf p.2
#guard (pairsNorm.filter fun p => allOf p.1 != allOf p.2).length = 17
-- Order against the 784 chosen matrices: patched 0, unpatched 62 (the normal forms
-- themselves: 0).
#guard crossDisagreements (withMF fixOf (pairsNorm.map (·.1))) chosenDataFix3 = 0
#guard crossDisagreements (withM allOf (pairsNorm.map (·.1))) chosenDataAll3 = 62
#guard crossDisagreements (withM allOf (pairsNorm.map (·.2))) chosenDataAll3 = 0

-- Point 5 (the step): `isOmegaForm α` is `α == [(.W v, 1)]` for the level `v` of `α`,
-- and `MstepFix` is `MstepAll` for the same `Mf`, on every label.
#guard everyLabel.all fun s =>
  let a := (parseM s).getD []
  isOmegaForm a == (match lvlO a with | some v => a == [(.W v, 1)] | none => false)
#guard everyLabel.all fun s =>
  let a := (parseM s).getD []
  MstepFix MfPrevF a == MstepAll MfPrevF a
