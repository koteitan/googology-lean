...
/-!
# Fix Fuel against the sheet, and the collision at depths 206/207

`TrioFixFuel.lean` is `TrioRulesAll.lean` (rules 1–10 of
[koteitan/trio](https://github.com/koteitan/trio) with Fixes A–E and N) with the
fuel `200` replaced by `fuelOf α = max 200 (odDep α)`, `odDep` the nesting depth.
This file re-runs every check of `TrioRulesAllSheet.lean` for the patched map
`trioRuleMatrixOfD` / `fxOf` / `MD` (Parts 1–7, the same guards with the same
numbers), and adds the checks of the fuel (Part 8).

* **Parts 1–7**: the sheet ([`tools/omega_alpha_rows.tsv`](https://github.com/koteitan/trio/blob/main/tools/omega_alpha_rows.tsv)):
  the 744 agreeing rows, the 22 `S` rows, the 9 `L` rows, the 8 `N`/`R` rows, rows
  3439, 3480, 4533; the order checks (783 and 784 chosen matrices, 0
  disagreements); the fixes' own guards (A–E, N); the interaction checks of Fixes
  E and N; the step-level comparison with rules 1–10 (same counts).
* **Part 8, the fuel.**  Every label used here has depth at most `8`, so the
  patched map is `TrioRulesAll`'s on all of them (theorem `trioRuleMatrixOfD_eq`,
  and a guard on the computed matrices).  Fuel `odDep α` alone (without the
  `max 200`) already gives the same matrix as fuel `fuelOf α + 200` on every
  label.  The collision of `TrioTree.lean` is gone: `ψ_0(Ω + T_205)` and
  `ψ_0(Ω + T_206)` (depths 206 and 207) get different matrices, in the right
  order, and on that family the patched map is `TrioTree.trioE` (the fuel-free
  map proved to keep the order and to land in the standard forms) at every depth
  checked, `190` to `300`, while the fuel-200 map leaves it from depth `201` on.
  The same below `ε₀` (towers of `ψ_0`, against `Trio.lean`'s `trioMatrix`) and
  for an uncountable family (`ω^ω^…^(Ω+1)`, depth up to `252`).

All of it is `#guard`s: a calibration, not a theorem (the theorems are in
`TrioFixFuel.lean`).
-/

namespace Googology.Trans.BMS.TrioFixFuelSheet
...
#guard trioRuleMatrixOfD "(W5)" -- row 2433
...
#guard trioRuleMatrixOfD "(W_2*3)" -- row 2725
...
#guard trioRuleMatrixOfD "(W_3+W_2)" -- row 2798
...
#guard trioRuleMatrixOfD "(W_3*2)" -- row 2799
...
#guard trioRuleMatrixOfD "W_6" -- row 2810
...
#guard trioRuleMatrixOfD "(W_w+W+2)" -- row 3368
...
#guard trioRuleMatrixOfD "(W_w+W+w)" -- row 3369
...
#guard trioRuleMatrixOfD "(W_w+W+w^2)" -- row 3370
...
#guard trioRuleMatrixOfD "(W_w+W2)" -- row 3371
...
#guard trioRuleMatrixOfD "(W_w+W*w)" -- row 3372
...
#guard trioRuleMatrixOfD "(W_w+W^2)" -- row 3373
...
#guard trioRuleMatrixOfD "(W_w+W^W)" -- row 3374
...
#guard trioRuleMatrixOfD "(W_w+psi_1(W_w))" -- row 3375
...
#guard trioRuleMatrixOfD "(W_w+W_2)" -- row 3376
...
#guard trioRuleMatrixOfD "(W_w+W_3)" -- row 3377
...
#guard trioRuleMatrixOfD "(W_w*W+w)" -- row 3479
...
#guard trioRuleMatrixOfD "(W_w*W*w)" -- row 3483
...
#guard trioRuleMatrixOfD "(W_w*W*w^2)" -- row 3484
...
#guard trioRuleMatrixOfD "(W_w*W*w^w)" -- row 3485
...
#guard trioRuleMatrixOfD "(W_w*W^2)" -- row 3486
...
#guard trioRuleMatrixOfD "(W_w*W^3)" -- row 3487
...
#guard trioRuleMatrixOfD "(W_w*W^w)" -- row 3488
...
#guard trioRuleMatrixOfD "(W_w*W^W)" -- row 3489
...
#guard trioRuleMatrixOfD "(W_w*psi_1(W_w))" -- row 3490
...
#guard trioRuleMatrixOfD "(W_w*W_2)" -- row 3491
...
#guard trioRuleMatrixOfD "(W_w*W_3)" -- row 3493
...
#guard trioRuleMatrixOfD "(W_(w+2)+w2)" -- row 3780
...
#guard trioRuleMatrixOfD "(W_(w+2)+w^2)" -- row 3781
...
#guard trioRuleMatrixOfD "(W_(w+2)+W)" -- row 3782
...
#guard trioRuleMatrixOfD "(W_(w+2)+W_w)" -- row 3783
...
#guard trioRuleMatrixOfD "(W_(w+2)+W_(w+1))" -- row 3784
...
#guard trioRuleMatrixOfD "(W_(w+2)*2)" -- row 3785
...
#guard trioRuleMatrixOfD "(W_(w+2)*3)" -- row 3786
...
#guard trioRuleMatrixOfD "(W_(w+3)*w)" -- row 3801
...
#guard trioRuleMatrixOfD "W_(w+4)" -- row 3802
...
#guard trioRuleMatrixOfD "(W_(w2+1)+w)" -- row 3915
...
#guard trioRuleMatrixOfD "(W_(w2+1)+w^2)" -- row 3916
...
#guard trioRuleMatrixOfD "(W_(w2+1)+W)" -- row 3917
...
#guard trioRuleMatrixOfD "(W_(w2+1)*2)" -- row 3918
...
#guard trioRuleMatrixOfD "(W_(w2+1)^2)" -- row 3921
...
#guard trioRuleMatrixOfD "(W_(w3)+w)" -- row 3943
...
#guard trioRuleMatrixOfD "(W_(w3)+w^2)" -- row 3944
...
#guard trioRuleMatrixOfD "(W_(w3)+W)" -- row 3945
...
#guard trioRuleMatrixOfD "(W_(w3)+W_(w2+1))" -- row 3946
...
#guard trioRuleMatrixOfD "(W_(w3)*2)" -- row 3947
...
#guard trioRuleMatrixOfD "(W_(w3)*w)" -- row 3948
...
#guard trioRuleMatrixOfD "(W_(w3)^2)" -- row 3949
...
#guard trioRuleMatrixOfD "psi_W_(w3+1)(W_(w3+1))" -- row 3950
...
#guard trioRuleMatrixOfD "(W_(w^2*2)+W_(w^2))" -- row 4333
...
#guard trioRuleMatrixOfD "(W_(w^2*2)+W_(w^2+1))" -- row 4334
...
#guard trioRuleMatrixOfD "(W_(w^2*2)*2)" -- row 4335
...
#guard trioRuleMatrixOfD "(W_W+2)" -- row 4452
...
#guard trioRuleMatrixOfD "(W_W+w)" -- row 4453
...
#guard trioRuleMatrixOfD "(W_W+w^2)" -- row 4454
...
#guard trioRuleMatrixOfD "(W_W+W)" -- row 4455
...
#guard trioRuleMatrixOfD "(W_W+W_2)" -- row 4456
...
#guard trioRuleMatrixOfD "(W_W+W_w)" -- row 4457
...
#guard trioRuleMatrixOfD "(W_W*2)" -- row 4458
...
#guard trioRuleMatrixOfD "(W_W*3)" -- row 4459
...
#guard trioRuleMatrixOfD "(W_W*W_2)" -- row 4462
...
#guard trioRuleMatrixOfD "(W_W*W_w)" -- row 4463
...
#guard trioRuleMatrixOfD "(W_W^2)" -- row 4464
...
#guard trioRuleMatrixOfD "(W_W^W_W)" -- row 4465
...
#guard trioRuleMatrixOfD "(W_(W+1)+w)" -- row 4486
...
#guard trioRuleMatrixOfD "(W_(W+1)+W_2)" -- row 4487
...
#guard trioRuleMatrixOfD "(W_(W+1)*2)" -- row 4489
...
#guard trioRuleMatrixOfD "psi_W_(W+2)(W_(W+2))" -- row 4492
...
#guard trioRuleMatrixOfD "W_(W+2)" -- row 4493
...
#guard trioRuleMatrixOfD "W_(W+3)" -- row 4494
...
#guard trioRuleMatrixOfD "W_(W+w^2)" -- row 4500
...
#guard trioRuleMatrixOfD "W_(W+w^w)" -- row 4501
...
#guard trioRuleMatrixOfD "W_(W2)" -- row 4502
...
#guard trioRuleMatrixOfD "W_(W3)" -- row 4503
...
#guard trioRuleMatrixOfD "(W_W_w+w)" -- row 4607
...
#guard trioRuleMatrixOfD "(W_W_w+W)" -- row 4608
...
#guard trioRuleMatrixOfD "(W_W_w+W_(w+1))" -- row 4610
...
#guard trioRuleMatrixOfD "(W_W_w+W_(w2))" -- row 4611
...
#guard trioRuleMatrixOfD "(W_W_w+W_W)" -- row 4612
...
#guard trioRuleMatrixOfD "(W_W_w*w)" -- row 4614
...
#guard trioRuleMatrixOfD "(W_W_w*W)" -- row 4615
...
#guard trioRuleMatrixOfD "(W_W_w*W_(w+1))" -- row 4616
...
#guard trioRuleMatrixOfD "(W_W_w*W_W)" -- row 4617
...
#guard trioRuleMatrixOfD "psi_W_(W_w+1)(W_(W_w+1))" -- row 4619
...
#guard trioRuleMatrixOfD "W_(W_w+w)" -- row 4622
...
#guard trioRuleMatrixOfD "W_(W_w*2)" -- row 4623
...
#guard trioRuleMatrixOfD "W_(W_w*3)" -- row 4624
...
#guard trioRuleMatrixOfD "(W_W_(w+1)*2)" -- row 4641
...
#guard trioRuleMatrixOfD "(W_W_(w+1)*w)" -- row 4642
...
#guard trioRuleMatrixOfD "(W_W_(w+1)^2)" -- row 4643
...
#guard trioRuleMatrixOfD "W_(W_(w+1)+1)" -- row 4644
...
#guard trioRuleMatrixOfD "W_(W_(w+1)*2)" -- row 4645
...
#guard trioRuleMatrixOfD "W_(W_(w+1)*w)" -- row 4646
...
#guard trioRuleMatrixOfD "W_(W_(w+1)^2)" -- row 4647
...
#guard trioRuleMatrixOfD "W_W_(w+2)" -- row 4648
...
#guard trioRuleMatrixOfD "W_W_(w3)" -- row 4651
...
#guard trioRuleMatrixOfD "W_W_(w^2)" -- row 4652
  = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1]]
#guard trioRuleMatrixOfD "(W_W_(w^2)+1)" -- row 4665
...
#guard trioRuleMatrixOfD "(W_W_(w^2)+W_(w^2+1))" -- row 4666
...
#guard trioRuleMatrixOfD "(W_W_(w^2)*w)" -- row 4668
...
#guard trioRuleMatrixOfD "W_(W_(w^2)+1)" -- row 4669
...
#guard trioRuleMatrixOfD "W_(W_(w^2)+W)" -- row 4670
...
#guard trioRuleMatrixOfD "W_(W_(w^2)*2)" -- row 4671
...
#guard trioRuleMatrixOfD "(W_W_W+w)" -- row 4741
...
#guard trioRuleMatrixOfD "(W_W_W+W_2)" -- row 4742
...
#guard trioRuleMatrixOfD "(W_W_W+W_w)" -- row 4743
...
#guard trioRuleMatrixOfD "(W_W_W*w)" -- row 4748
...
#guard trioRuleMatrixOfD "(W_W_W*W_2)" -- row 4749
...
#guard trioRuleMatrixOfD "psi_W_(W_W+1)(W_(W_W+1))" -- row 4754
...
#guard trioRuleMatrixOfD "W_(W_W+1)" -- row 4755
...
#guard trioRuleMatrixOfD "W_(W_W+w)" -- row 4756
...
#guard trioRuleMatrixOfD "W_(W_W+W_2)" -- row 4757
...
#guard trioRuleMatrixOfD "W_(W_W*2)" -- row 4758
...
#guard trioRuleMatrixOfD "W_(W_W*w)" -- row 4759
...
#guard trioRuleMatrixOfD "W_(W_W*W_2)" -- row 4760
...
#guard trioRuleMatrixOfD "W_(W_W^2)" -- row 4761
...
#guard trioRuleMatrixOfD "W_W_(W+1)" -- row 4763
...
#guard trioRuleMatrixOfD "W_W_(W+w)" -- row 4764
...
#guard trioRuleMatrixOfD "W_W_(W2)" -- row 4765
...
#guard trioRuleMatrixOfD "W_W_(W*w)" -- row 4766
...
#guard trioRuleMatrixOfD "W_W_(W^2)" -- row 4767
...
#guard trioRuleMatrixOfD "W_W_(W^W)" -- row 4768
...
#guard trioRuleMatrixOfD "W_W_W_2" -- row 4770
...
#guard trioRuleMatrixOfD "W_W_W_3" -- row 4771
...
#guard trioRuleMatrixOfD "W_W_W_w" -- row 4772
  = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1]]
#guard trioRuleMatrixOfD "W_W_W_(w+1)" -- row 4773
...
#guard trioRuleMatrixOfD "W_W_W_(w2)" -- row 4774
...
-- row 3492 (S-o)
#guard trioRuleMatrixOfD "(W_w*W_2*w)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[5,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,1],[6,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,1],[7,2,0],[6,4,1]]
...
-- row 4488 (S-o)
#guard trioRuleMatrixOfD "(W_(W+1)+W_W)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4490 (S-o)
#guard trioRuleMatrixOfD "(W_(W+1)*w)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,1]]
...
-- row 4491 (S-o)
#guard trioRuleMatrixOfD "(W_(W+1)^2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,1],[7,3,0]]
...
-- row 4497 (S-c)
#guard trioRuleMatrixOfD "(W_(W+w)*w)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1]]
...
-- row 4508 (S-o)
#guard trioRuleMatrixOfD "W_psi_1(W_2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[5,2,0]]
...
-- row 4609 (S-o)
#guard trioRuleMatrixOfD "(W_W_w+W_w)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[6,3,0],[7,4,1],[8,4,1],[9,1,0],[1,1,1]]
...
-- row 4613 (S-o)
#guard trioRuleMatrixOfD "(W_W_w*2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[6,3,0],[7,4,1],[8,4,1],[9,2,0],[3,2,1],[4,2,1],[5,1,0],[1,1,1]]
...
-- row 4618 (S-o)
#guard trioRuleMatrixOfD "(W_W_w^2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[6,3,1],[7,2,0],[3,2,1],[4,2,1],[5,1,0],[1,1,1]]
...
-- row 4628 (S-o)
#guard trioRuleMatrixOfD "W_psi_W_(w+1)(W_(w+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[6,2,0]]
...
-- row 4667 (S-c)
#guard trioRuleMatrixOfD "(W_W_(w^2)*2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[6,3,0],[7,4,1],[8,4,1],[9,2,0],[3,2,1],[4,2,1],[5,1,0],[1,1,1],[2,1,1]]
...
-- row 4674 (S-o)
#guard trioRuleMatrixOfD "W_(psi_W_(w^2+1)(W_(w^2+1)))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,1,0],[6,2,0]]
...
-- row 4744 (S-c)
#guard trioRuleMatrixOfD "(W_W_W+W_W)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4745 (S-c)
#guard trioRuleMatrixOfD "(W_W_W+W_(W+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0]]
...
-- row 4746 (S-c)
#guard trioRuleMatrixOfD "(W_W_W+W_W_2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0]]
...
-- row 4747 (S-c)
#guard trioRuleMatrixOfD "(W_W_W*2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4750 (S-c)
#guard trioRuleMatrixOfD "(W_W_W*W_W)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1],[8,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4751 (S-c)
#guard trioRuleMatrixOfD "(W_W_W*W_(W+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1],[8,3,0]]
...
-- row 4752 (S-c)
#guard trioRuleMatrixOfD "(W_W_W*W_W_2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1],[8,3,0],[4,3,1],[5,3,1],[6,2,0]]
...
-- row 4753 (S-c)
#guard trioRuleMatrixOfD "(W_W_W^2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,1],[8,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,1],[3,2,1],[4,1,0]]
...
-- row 4762 (S-o)
#guard trioRuleMatrixOfD "W_psi_W_(W+1)(W_(W+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[7,3,0]]
...
-- row 4769 (S-o)
#guard trioRuleMatrixOfD "W_W_psi_1(W_2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[5,2,0]]
...
#guard trioRuleMatrixOfD "(w^5*2)" = some [[0,0,0],[1,1,1],[2,1,1],[2,1,1],[2,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[4,2,1],[4,2,1],[4,2,1]]
#guard trioRuleMatrixOfD "(w^5*3)" = trioRuleMatrixOf "(w^5*3)"
-- row 2131, printed `(w^w+w2)`, corrected `(w^w+2)`
#guard trioRuleMatrixOfD "(w^w+2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,0,0],[2,1,0],[3,2,0],[4,3,0]]
#guard trioRuleMatrixOfD "(w^w+w2)" = trioRuleMatrixOf "(w^w+w2)"
-- row 2532, printed `(W^(W+1))`, corrected `(W^(w+1))`
#guard trioRuleMatrixOfD "(W^(w+1))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,1,0],[4,0,0],[3,2,1],[4,1,0]]
#guard trioRuleMatrixOfD "(W^(W+1))" = trioRuleMatrixOf "(W^(W+1))"
-- row 2723, printed `(W_2*2+W)`, corrected `(W_2*2+w^2)`
#guard trioRuleMatrixOfD "(W_2*2+w^2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,0],[3,3,1],[4,3,1],[5,2,0],[4,3,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,1],[8,5,1]]
#guard trioRuleMatrixOfD "(W_2*2+W)" = trioRuleMatrixOf "(W_2*2+W)"
-- row 3452, printed `(W_w*w+W_w+10)`, corrected `(W_w*w+W_w+1)`
#guard trioRuleMatrixOfD "(W_w*w+W_w+1)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[4,2,0],[5,3,1],[6,3,1],[7,1,0],[6,3,0],[7,4,0]]
#guard trioRuleMatrixOfD "(W_w*w+W_w+10)" = trioRuleMatrixOf "(W_w*w+W_w+10)"
-- row 3552, printed `psi_W_(W+1)(W_psi_W_(w+1)(W_(w+1)))`, corrected `psi_W_(w+1)(W_psi_W_(w+1)(W_(w+1)))`
#guard trioRuleMatrixOfD "psi_W_(w+1)(W_psi_W_(w+1)(W_(w+1)))" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[6,2,1],[7,2,1],[8,1,0],[9,2,0]]
-- Fix E: the printed label now gets `TrioRules3`'s matrix, not rules 1–10's.
#guard trioRuleMatrixOfD printed3552 = trioRuleMatrixOf3 printed3552
#guard trioRuleMatrixOfD printed3552 ≠ trioRuleMatrixOf printed3552
-- row 3777, printed `(W_(w+2)+w)`, corrected `(W_(w+2)+2)`
#guard trioRuleMatrixOfD "(W_(w+2)+2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,0],[8,6,0]]
#guard trioRuleMatrixOfD "(W_(w+2)+w)" = trioRuleMatrixOf "(W_(w+2)+w)"
-- row 4369, printed `(W_(w^3*W))`, corrected `(W_(w^3)*W)`
#guard trioRuleMatrixOfD "(W_(w^3)*W)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[5,1,0]]
#guard trioRuleMatrixOfD "(W_(w^3*W))" = trioRuleMatrixOf "(W_(w^3*W))"
-- row 4384, printed `W_(w^2*2)`, corrected `W_(w^w*2)`
#guard trioRuleMatrixOfD "W_(w^w*2)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,0,0],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,1],[5,0,0]]
#guard trioRuleMatrixOfD "W_(w^2*2)" = trioRuleMatrixOf "W_(w^2*2)"

/-! ### The 8 rows where the rules are right and the sheet has another matrix -/

-- row 2133 (R)
#guard trioRuleMatrixOfD "(w^w+w2)" = trioRuleMatrixOf "(w^w+w2)"
#guard trioRuleMatrixOfD "(w^w+w2)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,0,0],[2,1,0],[3,2,1],[3,2,0],[4,3,1]]
-- row 3453 (N)
#guard trioRuleMatrixOfD "(W_w*w+W_w*2)" = trioRuleMatrixOf "(W_w*w+W_w*2)"
#guard trioRuleMatrixOfD "(W_w*w+W_w*2)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[4,2,0],[5,3,1],[6,3,1],[7,1,0],[6,3,0],[7,4,1],[8,4,1],[9,1,0]]
-- row 3551 (R (weak))
#guard trioRuleMatrixOfD "psi_W_(w+1)(W_W_w)" = trioRuleMatrixOf "psi_W_(w+1)(W_W_w)"
#guard trioRuleMatrixOfD "psi_W_(w+1)(W_W_w)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[6,2,1],[7,2,1],[8,1,0],[1,1,1]]
-- row 3706 (N)
#guard trioRuleMatrixOfD "(W_(w+1)+W_w^2)" = trioRuleMatrixOf "(W_(w+1)+W_w^2)"
#guard trioRuleMatrixOfD "(W_(w+1)+W_w^2)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,1,0],[8,1,0],[1,1,1]]
-- row 3709 (N)
#guard trioRuleMatrixOfD "(W_(w+1)*2+W)" = trioRuleMatrixOf "(W_(w+1)*2+W)"
#guard trioRuleMatrixOfD "(W_(w+1)*2+W)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,2,0],[7,4,0],[8,5,1]]
-- row 3923 (R)
#guard trioRuleMatrixOfD "psi_W_(w2+2)(W_(w2+2))" = trioRuleMatrixOf "psi_W_(w2+2)(W_(w2+2))"
#guard trioRuleMatrixOfD "psi_W_(w2+2)(W_(w2+2))" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,0],[5,3,1],[6,3,1],[7,3,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[9,3,0]]
-- row 4303 (N)
#guard trioRuleMatrixOfD "(W_(w^2+w)^W_(w^2+w))" = trioRuleMatrixOf "(W_(w^2+w)^W_(w^2+w))"
#guard trioRuleMatrixOfD "(W_(w^2+w)^W_(w^2+w))" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[2,1,0],[3,2,1],[4,2,1],[5,2,0],[3,2,1],[4,2,0],[5,3,1],[6,3,1],[7,2,0],[8,2,0],[3,2,1]]
-- row 4496 (N)
#guard trioRuleMatrixOfD "(W_(W+w)+1)" = trioRuleMatrixOf "(W_(W+w)+1)"
#guard trioRuleMatrixOfD "(W_(W+w)+1)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1]]

/-! ### Rows 3439 and 3480: unchanged -/

-- row 3439 (X)
#guard trioRuleMatrixOfD "(W_w*w0*W_W_w+W_(W_w*w))" = trioRuleMatrixOf "(W_w*w0*W_W_w+W_(W_w*w))"
#guard trioRuleMatrixOfD "(W_w*w0*W_W_w+W_(W_w*w))" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[4,2,0],[3,2,1],[4,2,1],[5,1,0],[4,2,0],[5,3,1],[6,3,1],[7,1,0],[6,3,1],[6,2,0],[5,3,1],[6,3,1],[7,1,0],[6,3,1]]
-- row 3480 (O)
#guard trioRuleMatrixOfD "(W_w*W+W_3)" = trioRuleMatrixOf "(W_w*W+W_3)"
#guard trioRuleMatrixOfD "(W_w*W+W_3)" ≠ some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,0],[3,2,1],[4,2,1],[5,1,0],[4,2,1],[5,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,1],[6,1,0],[5,3,0],[6,4,1],[7,4,1],[8,1,0],[7,4,1]]
...
#guard trioRuleMatrixOfD "W_psi_2(W_3)" = some [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,2,0],[6,3,0]]
...
def chosenDataD : Array (Od × List (List Nat)) :=
  (chosen.map fun s => ((parse s).getD [], fxOf s)).toArray

#guard chosenDataD.size = 783
#guard chosenDataD.all fun p => !p.1.isEmpty && !p.2.isEmpty
#guard orderDisagreements chosenDataD = 0
...
#guard allLabels.all fun s => trioRuleMatrixOfD s == trioRuleMatrixOf3 s
#guard (allLabels.filter (· != printed3552)).all fun s => trioRuleMatrixOfD s == trioRuleMatrixOf2 s
#guard trioRuleMatrixOfD printed3552 != trioRuleMatrixOf2 printed3552

/-- The 783 labels and the printed label of row 3552, with the patched matrices. -/
def chosenDataD3 : Array (Od × List (List Nat)) :=
  ((chosen ++ [printed3552]).map fun s => ((parse s).getD [], fxOf s)).toArray

#guard chosenDataD3.size = 784
#guard chosenDataD3.all fun p => !p.1.isEmpty && !p.2.isEmpty
#guard orderDisagreements chosenDataD3 = 0

#guard fxOf printed3552 = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[7,3,1],[8,3,1],[9,2,0],[2,2,1],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[7,3,0]]
#guard fxOf printed3552 =
  fxOf "psi_W_(W+1)(W_W_2)" ++ liftRows ((fxOf "psi_W_(w+1)(W_(w+1))").drop 4)
#guard fxOf "psi_W_(W+1)(W_W_2)" < fxOf "psi_W_(W+1)(W_W_w)"
#guard fxOf "psi_W_(W+1)(W_W_w)" < fxOf printed3552
#guard fxOf printed3552 < fxOf "W_(W+1)"
#guard (List.range 4).all fun n =>
  expandRL 3 n (fxOf printed3552) =
    fxOf "psi_W_(W+1)(W_W_2)" ++
      liftRows ((expandRL 3 n (fxOf "psi_W_(w+1)(W_(w+1))")).drop 4)
#guard fxOf "psi_W_(W+1)(W_W_w)" = fxOf "psi_W_(W+1)(W_W_2)" ++ [[2,2,1]]
#guard expandRL 3 1 (fxOf "psi_W_(W+1)(W_W_w)") =
  fxOf "psi_W_(W+1)(W_W_2)" ++
    [[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[8,4,1],[9,4,1],[10,3,0]]
#guard expandRL 3 2 (fxOf "W_(W+1)") = fxOf "psi_W_(W+1)(W_W_2)" ++ [[10,3,1],[11,3,1]]
#guard fxOf "psi_W_(W+1)(W_psi_W_(W+1)(W_(W+1)))" = fxOf "psi_W_(W+1)(W_W_2)" ++ [[10,3,0]]
#guard expandRL 3 2 (fxOf "psi_W_(W+1)(W_psi_W_(W+1)(W_(W+1)))") =
  fxOf "psi_W_(W+1)(W_W_2)" ++ [[10,2,0],[11,2,0]]
#guard fxOf printed3552 < fxOf "psi_W_(W+1)(W_psi_W_(W+1)(W_(W+1)))"
#guard fxOf "psi_W_(w+1)(W_W)" = row3550
#guard fxOf "psi_W_(w+1)(W_W_w)" = row3550 ++ [[6,2,1]]
...
#guard (sheetLabels.filter (· != printed3552)).all fun s => trioRuleMatrixOfD s == trioRuleMatrixOfN s
#guard sheetLabels.all fun s => trioRuleMatrixOfD s == trioRuleMatrixOf3 s

-- Fix N on the plan's case, the lifted copy, and BMS expansion.
#guard fxOf "(W_W_W+W_W_2+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,4,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,4,0],[8,5,0],[9,6,1],[10,6,1],[11,4,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,0]]
#guard fxOf "(W_W_W+W_(W+1)+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,0],[5,4,1],[6,4,1],[7,4,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,1],[8,5,1],[9,4,0],[8,5,0],[9,6,1],[10,6,1],[11,3,0],[10,6,0],[11,7,0]]
#guard fxOf "(W_W_W*2+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[9,5,0],[10,6,0]]
#guard fxOf "(W_W+W_2+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,2,0],[8,5,0],[9,6,0]]
#guard fxOf "(W_W*2+1)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,2,0],[7,4,0],[8,5,0]]
#guard fxOf "(W_W_W+W_W_2*2)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,4,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,4,0],[8,5,0],[9,6,1],[10,6,1],[11,4,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,1],[8,5,1],[9,3,0],[5,4,1],[6,4,1],[7,2,0]]
#guard fxOf "(W_W_W+W_W_2*w)" = [[0,0,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,1],[2,1,1],[3,1,0],[1,1,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,2,0],[2,2,1],[3,2,1],[4,1,0],[3,2,0],[4,3,1],[5,3,1],[6,3,0],[4,3,1],[5,3,1],[6,2,0],[5,3,0],[6,4,1],[7,4,1],[8,3,0],[7,4,0],[8,5,1],[9,5,1],[10,3,0],[4,3,1],[5,3,1],[6,2,0],[2,2,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,3,0],[3,3,1],[4,3,1],[5,1,0],[4,3,0],[5,4,1],[6,4,1],[7,4,0],[5,4,1],[6,4,1],[7,3,0],[6,4,0],[7,5,1],[8,5,1],[9,4,0],[8,5,0],[9,6,1],[10,6,1],[11,4,0],[5,4,1],[6,4,1],[7,2,0],[6,4,0],[7,5,1],[8,5,1],[9,3,0],[8,5,1]]
...
#guard cmpMat (fxOf "(W_W_W+W_W_2)") (fxOf "(W_W_W+W_W_2+1)") = .lt
#guard cmpMat (fxOf "(W_W_W+W_W_2+1)") (fxOf "(W_W_W+W_W_2*2)") = .lt
#guard cmpMat (fxOf "(W_W_W+W_W_2*2)") (fxOf "(W_W_W*2)") = .lt
#guard cmpMat (fxOf "(W_W_W+W_(W+1)+1)") (fxOf "(W_W_W+W_W_2)") = .lt
#guard cmpMat (fxOf "(W_W_W*2)") (fxOf "(W_W_W*2+1)") = .lt
#guard cmpMat (fxOf "(W_W+W_2+1)") (fxOf "(W_W*2)") = .lt
#guard cmpMat (fxOf "(W_W+W_2+1)") (fxOf "(W_W+W_w)") = .lt
#guard expandRL 3 1 (fxOf "(W_W_W+W_W_w)") =
  toRows (MD ((parse "(W_W_W+W_W_2)").getD []) ++ bliftUp (MD ((parse "(W_W_W+W_W_2)").getD [])) 10)
#guard expandRL 3 1 (fxOf "(W_W_W+W_w)") =
  toRows (MD ((parse "(W_W_W+W_2)").getD []) ++ bliftUp (MD ((parse "(W_W_W+W_2)").getD [])) 10)
#guard expandRL 3 1 (fxOf "(W_W+W_w)") =
  toRows (MD ((parse "(W_W+W_2)").getD []) ++ bliftUp (MD ((parse "(W_W+W_2)").getD [])) 7)
#guard expandRL 3 1 (fxOf "(W_W*w)") = fxOf "(W_W+W_2)"
#guard expandRL 3 2 (fxOf "(W_W*w)") = fxOf "(W_W*2+W_2)"
#guard expandRL 3 1 (fxOf "(W_W_W*w)") = fxOf "(W_W_W+W_(W+1))"
#guard expandRL 3 2 (fxOf "(W_W_W*w)") = fxOf "(W_W_W*2+W_(W+1))"
#guard expandRL 3 1 (fxOf "(W_W_W^2*w)") = fxOf "(W_W_W^2+W_W_W*W_(W+1))"
#guard expandRL 3 2 (fxOf "(W_W_W^2*w)") = fxOf "(W_W_W^2*2+W_W_W*W_(W+1))"
#guard expandRL 3 1 (fxOf "(W_W_W+W_2*w)") = fxOf "(W_W_W+W_2*2)"
#guard expandRL 3 2 (fxOf "(W_W_W+W_2*w)") = fxOf "(W_W_W+W_2*3)"
#guard expandRL 3 1 (fxOf "(W_W_W+W_(W+1)*w)") = fxOf "(W_W_W+W_(W+1)*2)"
#guard expandRL 3 2 (fxOf "(W_W_W+W_(W+1)*w)") = fxOf "(W_W_W+W_(W+1)*3)"
#guard expandRL 3 1 (fxOf "(W_W_W+W_W_2*w)") = fxOf "(W_W_W+W_W_2*2+W_(W+1))"
#guard expandRL 3 2 (fxOf "(W_W_W+W_W_2*w)") = fxOf "(W_W_W+W_W_2*3+W_(W+1))"
#guard expandRL 3 1 (fxOf "(W_W_W*W_2*w)") = fxOf "(W_W_W*W_2*2)"
#guard expandRL 3 2 (fxOf "(W_W_W*W_2*w)") = fxOf "(W_W_W*W_2*3)"
#guard expandRL 3 1 (fxOf "(W_W_W*W_(W+1)*w)") = fxOf "(W_W_W*W_(W+1)*2)"
#guard expandRL 3 2 (fxOf "(W_W_W*W_(W+1)*w)") = fxOf "(W_W_W*W_(W+1)*3)"
#guard expandRL 3 1 (fxOf "(W_W_W*W_W_2*w)") = fxOf "(W_W_W*W_W_2*2+W_W_W*W_(W+1))"
#guard expandRL 3 1 (fxOf "(W_W_W+W_W_2+W_2*w)") = fxOf "(W_W_W+W_W_2+W_2*2)"
#guard expandRL 3 2 (fxOf "(W_W_W+W_W_2+W_2*w)") = fxOf "(W_W_W+W_W_2+W_2*3)"
#guard expandRL 3 1 (fxOf "(W_W_W+W_(W+1)+W_2*w)") = fxOf "(W_W_W+W_(W+1)+W_2*2)"
#guard expandRL 3 2 (fxOf "(W_W_W+W_(W+1)+W_2*w)") = fxOf "(W_W_W+W_(W+1)+W_2*3)"
#guard expandRL 3 1 (fxOf "(W_W_W+W_W_2+W_(W+1)*w)") = fxOf "(W_W_W+W_W_2+W_(W+1)*2)"
#guard expandRL 3 2 (fxOf "(W_W_W+W_W_2+W_(W+1)*w)") = fxOf "(W_W_W+W_W_2+W_(W+1)*3)"
#guard expandRL 3 1 (fxOf "(W_W+W_2*w)") = fxOf "(W_W+W_2*2)"
#guard expandRL 3 2 (fxOf "(W_W+W_2*w)") = fxOf "(W_W+W_2*3)"
#guard expandRL 3 1 (fxOf "(W_W+W_2+W*w)") = fxOf "(W_W+W_2+W*2)"
#guard expandRL 3 2 (fxOf "(W_W+W_2+W*w)") = fxOf "(W_W+W_2+W*3)"

-- The nearby families: the same counts as with Fix N alone.
#guard pairDisagreements (withM fxOf familyW) = 1 ∧ collisions (withM fxOf familyW) = 1
#guard pairDisagreements (withM fxOf family1) = 0 ∧ collisions (withM fxOf family1) = 0
#guard pairDisagreements (withM fxOf probes) = 25
#guard crossDisagreements (withM fxOf probes) chosenDataD = 20
#guard crossDisagreements (withM fxOf probes) chosenDataD3 = 20

/-! ## Part 5: against rules 1–10 and the single-fix versions

`interactions ls` lists the labels of `ls` where the merged matrix is not the one
...
def interactions (ls : List String) : List String :=
  ls.filter fun s =>
    let a := fxOf s
    let o := ruleOf2 s
    let e := ruleOf3 s
...
#guard interactions sheetLabels = []
#guard interactions familyW = []
#guard interactions family1 = []
#guard interactions probes = []

-- Against rules 1–10: 24 labels differ (22 `S` rows, row 4533, printed row 3552);
-- `TrioRules2` differs on 23 of them.
#guard (sheetLabels.filter fun s => trioRuleMatrixOfD s != trioRuleMatrixOf s).length = 24
#guard (sheetLabels.filter fun s => trioRuleMatrixOf2 s != trioRuleMatrixOf s).length = 23
#guard (sheetLabels.filter fun s => trioRuleMatrixOfD s != trioRuleMatrixOf s).all fun s =>
...
def meet (s : String) : Bool × Bool × Bool × Bool :=
  (ruleOf3 s != ruleOf2 s, nOf s != ruleOf2 s, fxOf s == ruleOf3 s, fxOf s == nOf s)
...
#guard interactions probesEN = [
  ...
  "psi_W_(W+1)(W_psi_W_(w+1)(W_(W_W+W_2+1)))",
  "psi_W_(W+1)(W_psi_W_(w+1)(W_(W_W_W+W_(W+1)+1)))"]
#guard (interactions probesEN).all fun s => meet s == (true, true, false, false)
-- On the other 12, only one fix acts and the merged matrix is that version's.
#guard (probesEN.filter fun s => !(interactions probesEN).contains s).all fun s =>
  let m := meet s
...
-- collisions).
#guard crossDisagreements (withM fxOf probesEN) chosenDataD3 = 0
#guard crossDisagreements (withM nOf probesEN)
  (((chosen ++ [printed3552]).map fun s => ((parse s).getD [], nOf s)).toArray) = 5
#guard pairDisagreements (withM fxOf probesEN) = 3 ∧ collisions (withM fxOf probesEN) = 3
#guard pairDisagreements (withM ruleOf3 probesEN) = 11 ∧ collisions (withM ruleOf3 probesEN) = 8
#guard fxOf "psi_W_(W+1)(W_W_w)" = fxOf "psi_W_(W+1)(W_W_w+1)"

/-! ## Part 7: one step against rules 1–10

As in `TrioRulesAllSheet.lean`, with the patched builder one step of fuel down
(`MfPrev α`, fuel `fuelOf α`) and `TrioFuel.Mstep` (rules 1–10's step with the
same fuel).  The counts are the same. -/

/-- Every label used in this file. -/
def corpus : List String := sheetLabels ++ familyW ++ family1 ++ probes ++ probesEN

/-- The patched builder one step of fuel down. -/
def MfPrev (a : Od) : Od → Cols := TrioFixFuel.MfuelAll (fuelOf a) (fuelOf a - 1)

/-- The branch of the builder that `α` takes. -/
def branch (a : Od) : String :=
  match lvlO a with
  | none => "countable"
...
def stepDiffers (s : String) : Bool :=
  let a := (parse s).getD []
  TrioFixFuel.MstepAll (fuelOf a) (MfPrev a) a != TrioFuel.Mstep (fuelOf a) (MfPrev a) a

/-- `MD α` is one patched step on `MfPrev α`. -/
theorem MD_eq_step (a : Od) : MD a = TrioFixFuel.MstepAll (fuelOf a) (MfPrev a) a := by
  obtain ⟨k, hk⟩ : ∃ k, fuelOf a = k + 1 :=
    ⟨fuelOf a - 1, by have := le_max_left fuel (odDep a); simp only [fuelOf, fuel] at this ⊢; omega⟩
  simp only [MD, TrioFixFuel.MF, MfPrev, hk, Nat.add_sub_cancel]
  rfl

#guard (["countable", "psi", "omega", "sum"].map fun k =>
    let ls := corpus.filter fun s => branch ((parse s).getD []) == k
    (k, ls.length, (ls.filter stepDiffers).length)) =
  [("countable", 171, 0), ("psi", 32, 6), ("omega", 152, 7), ("sum", 870, 257)]
#guard (corpus.filter fun s => branch ((parse s).getD []) == "psi").all fun s =>
  let a := (parse s).getD []
  match isPsiLevel a with
  | some uX => stepDiffers s ==
      (TrioRulesAll.fixAFires (MfPrev a) uX.1 || TrioRulesAll.fixEFires (MfPrev a) uX.1 uX.2)
  | none => false
#guard (corpus.filter fun s => branch ((parse s).getD []) == "omega").all fun s =>
  let a := (parse s).getD []
  match lvlO a with
  | some v => stepDiffers s == TrioRulesAll.fixAFires (MfPrev a) v
  | none => false

/-! ## Part 8: the fuel

### On the sheet nothing changes

Every label of `corpus` (1,225 labels with repeats) has depth at most `8`, so
`trioRuleMatrixOfD_eq` gives `trioRuleMatrixOfD s = trioRuleMatrixOfAll s`; the
guard below checks the computed matrices too.  The depth alone is enough fuel:
fuel `odDep α` gives the matrix of fuel `fuelOf α + 200`. -/

#guard (corpus.map fun s => odDep ((parse s).getD [])).foldl max 0 = 8
#guard corpus.all fun s => trioRuleMatrixOfD s == TrioRulesAll.trioRuleMatrixOfAll s
#guard corpus.all fun s =>
  let a := (parse s).getD []
  MF (odDep a) a == MF (fuelOf a + 200) a

/-! ### The collision of `TrioTree.lean` is gone

`fam n = ψ_0(Ω + T_n)`, `T_n` the tower of `n` copies of `ψ_0`, depth `n + 1`.
`TrioTree.trioE` is the map without fuel on the terms whose subscripts are `0` or
`1`; `TrioTree.lean` proves that it keeps the order and `TrioTreeStd.lean` that it
lands in the standard forms of three-row BMS. -/

open Googology.Trans.BMS.TrioTree (trioE towerP)
open Googology.Trans.BMS.TrioRulesE0 (dep)

/-- `ψ_0(Ω + T_n)`. -/
def fam (n : Nat) : Term := psi nil (cons t1 nil (towerP n))

#guard dep (fam 205) = 206 && dep (fam 206) = 207
#guard TrioFixFuel.tDep (fam 205) = 206 && TrioFixFuel.tDep (fam 206) = 207
#guard odDep (ofTermD (fam 205)) = 206 && odDep (ofTermD (fam 206)) = 207
-- The fuel-200 map (`TrioRulesAll`) gives the two terms one matrix; the patched map
-- does not, and orders them.
#guard trioMatrixLAll (fam 205) = trioMatrixLAll (fam 206)
#guard trioMatrixLD (fam 205) ≠ trioMatrixLD (fam 206)
#guard trioMatrixLD (fam 205) < trioMatrixLD (fam 206)
-- The patched map is `trioE` at depths 191, 201, …, 301; the fuel-200 map only at 191.
#guard (List.range 12).all fun k => trioMatrixLD (fam (190 + 10 * k)) == trioE (fam (190 + 10 * k))
#guard ((List.range 12).filter fun k =>
  trioMatrixLAll (fam (190 + 10 * k)) == trioE (fam (190 + 10 * k))) = [0]
-- The fuel-200 map is `trioE` up to depth 200, then not.
#guard trioMatrixLAll (fam 199) = trioE (fam 199) && trioMatrixLAll (fam 200) ≠ trioE (fam 200)
-- Consecutive depths 191–251: strictly increasing matrices.
#guard (List.range 60).all fun k => trioMatrixLD (fam (190 + k)) < trioMatrixLD (fam (191 + k))

/-! ### Below `ε₀`: towers of `ψ_0`

`TrioRulesFuel.lean` proves that rules 1–10 with fuel `dep α + 1` are `trioMatrix`
below `ε₀`; the patched merged map agrees there too (checked, not proved). -/

#guard trioMatrixLAll (towerP 210) = trioMatrixLAll (towerP 211)
#guard trioMatrixLD (towerP 210) ≠ trioMatrixLD (towerP 211)
#guard [202, 210, 211, 260].all fun n =>
  trioMatrixLD (towerP n) == Googology.Trans.BMS.trioMatrix (towerP n)

/-! ### An uncountable family: `ω^ω^…^(Ω+1)`

`wTowerOm n` is `Ω + 1` under `n` exponentials `ω^·`, depth `n + 2`.  The ordinals
increase with `n`. -/
...
#guard [198, 199, 200, 250].map (fun n => odDep (wTowerOm n)) = [200, 201, 202, 252]
-- Fuel 200 is right up to depth 200 and wrong from 201 on (it no longer changes
-- with `n`); the patched map keeps growing, is strictly increasing, and more fuel
-- changes nothing.
#guard MD (wTowerOm 198) = TrioRulesAll.MAll (wTowerOm 198)
#guard [199, 200, 250].all fun n => MD (wTowerOm n) != TrioRulesAll.MAll (wTowerOm n)
-- At depth 201 fuel 200 no longer finds the level `Ω_1` (`lvlO` runs out and
-- reads the ordinal as countable), and from depth 203 on the fuel-200 matrices
-- are in the wrong order.
#guard lvlO (wTowerOm 198) == some one && lvlO (wTowerOm 199) == none
#guard toRows (TrioRulesAll.MAll (wTowerOm 202)) < toRows (TrioRulesAll.MAll (wTowerOm 201))
#guard (List.range 12).all fun k =>
  toRows (MD (wTowerOm (195 + k))) < toRows (MD (wTowerOm (196 + k)))
#guard [199, 200, 250].all fun n => MD (wTowerOm n) == MF (fuelOf (wTowerOm n) + 50) (wTowerOm n)

end Googology.Trans.BMS.TrioFixFuelSheet
