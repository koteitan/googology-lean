...
See `TrioFixStripSheet.lean` for the summary.  Part 9, the 2,397 standard countable terms with at most 7 `ψ`s and
subscripts `0`/`1`: 0 order disagreements (127 with `trioMatrixLFix`), 0 collisions,
the patched map differs from `trioMatrixLFix` only on `β`, and the reading
`ofTermFix` keeps the order.
...
#guard (badPairs frag7 trioMatrixLSt).length = 0
#guard collide frag7 trioMatrixLSt = 0
#guard (frag7.filter fun a => trioMatrixLSt a != trioMatrixLFix a) = [tβ]
