...
| BMS at three rows | `BMS/TrioFixNonLastOther2.lean`, `BMS/TrioFixNonLastOther2Sheet*.lean` | [koteitan/trio](https://github.com/koteitan/trio) | patch on Fix nonlast-other: Fix M, Fix G and change 1 act only where the leaf of the named level is apart from the lower tops (changes 1′, G′, M′). All nine regressions found in review disappear, the five calibrated families keep their matrices, and no standard matrix of `TrioRulesAll` becomes non-standard on 3,387 labels; some pairs after `Ω_{Ω_{ω+2}}` and similar levels are still out of order ([TRIO-FIX-NONLAST2.md](BMS/TRIO-FIX-NONLAST2.md)) |
