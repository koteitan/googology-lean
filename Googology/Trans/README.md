...
| DBMS with itself | `DBMS/BlocksLex.lean` | — | **reachability and the lexicographic order** (any number of rows): one expansion step makes a list of columns lexicographically smaller (`expandRL_lt_self`), so reaching implies `≤`. Standard ⟺ contents lexicographically non-increasing holds iff `LexReach r` (`lexReach_iff_dstdL_iff_dlex`); at 3 rows the necessary direction is proved (`dstdL_three_dlex`) and `LexReach 2` is open |
