...
| BMS, extended Buchholz's ψ | `BMS/EpsN.lean` | — | **that `val` is onto the ordinals below every `ε_n`**, by induction on `n` with the `ε₁` construction as its step: `Ω·n + B` is a term, and `ψ_0(Ω·(n+1) + B)` is the leading one |
