...
| DBMS、BMS | `DBMS/ThreeRowUpperNC*.lean`、`DBMS/ThreeRowUpperRP*.lean` | — | **n = 2 を 1 つの命題に帰着**。入れ子の持ち上げ `t3n` で、可換でない場合を上げられる親の位置で分けた。最後から 2 つ目の列の場合と `RPLastShape` は証明済み。`rkL 2 (cgen 2 4) = rkL 2 (bgen3 2)` は `T3nPushStd`（未解決：`t3n C` がトリオの標準形なら `t3n (pushL C y)` もそう。約 160 万の行列で失敗 0）から出る（`rkL_cgen_two_four_eq_of_push`） |
