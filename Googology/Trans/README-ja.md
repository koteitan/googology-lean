...
| DBMS どうし | `DBMS/BlocksLex.lean` | — | **「届く」と辞書式の順序**（何行でも）。1 回の展開で列の並びは辞書式に小さくなる（`expandRL_lt_self`）ので、届けば `≤`。「標準形 ⟺ 中身が辞書式に増えない」は `LexReach r` と同値（`lexReach_iff_dstdL_iff_dlex`）。3 行では必要の向きを証明済み（`dstdL_three_dlex`）、`LexReach 2` は未解決 |
